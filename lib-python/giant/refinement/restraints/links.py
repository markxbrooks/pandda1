...
class LinkRecord(object):
    ...
    def _parse(self, link_record):
        ...
        if len(link_record) < 80:
            link_record = link_record + " " * (80 - len(link_record))

        self.set_link_type(link_record[0:6].strip(" "))

        self.atom_1_name = link_record[12:16].strip(" ")
        self.atom_1_altloc = link_record[16].strip(" ")
        self.atom_1_resname = link_record[17:20].strip(" ")
        self.atom_1_chain = link_record[21].strip(" ")
        self.atom_1_resseq = link_record[22:26].strip(" ")
        self.atom_1_inscode = link_record[26].strip(" ")

        self.atom_2_name = link_record[42:46].strip(" ")
        self.atom_2_altloc = link_record[46].strip(" ")
        self.atom_2_resname = link_record[47:50].strip(" ")
        self.atom_2_chain = link_record[51].strip(" ")
        self.atom_2_resseq = link_record[52:56].strip(" ")
        self.atom_2_inscode = link_record[56].strip(" ")

        self.sym_op_1 = link_record[59:65].strip(" ")
        self.sym_op_2 = link_record[66:72].strip(" ")

        if self.link_type == "LINK":
            self.set_link_dist(link_record[73:78].strip(" "))
        else:
            self.set_link_id(link_record[72:80].strip(" "))
    ...
    def __call__(self, link_records):
        ...
        for l in link_records:
            ...
            if l.link_id is not None:
                ...
                strings.append(self.format_link_record(l))

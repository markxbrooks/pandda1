...
def set_occupancy(atoms, occupancy):
    ...
    atoms.set_occ(flex.double(atoms.size(), occupancy))
...
class ScaleOccupancies(object):
    ...
    def __call__(self, atoms, multiplier):
        ...
        atoms.set_occ(atoms.extract_occ() * float(multiplier))
...
class ResetOccupancies(object):
    ...
    def __call__(self, hierarchy):
        ...
        n_altlocs = len([a for a in altloc_indices if a.strip()])
        ...
        if n_altlocs > 0:
            ...
            alt_occupancy = self.default_occupancy / float(n_altlocs)
...
class SanitiseOccupancies(object):
    ...
    def scale_main_to_max(self, hierarchy):
        ...
        import numpy as np
        main_sel_array = np.array(main_sel)
        if main_sel_array.ndim == 0:
            main_indices = flex.size_t([0]) if main_sel_array else flex.size_t([])
        else:
            main_indices = flex.size_t([int(i) for i in np.where(main_sel_array)[0]])
        main_atoms = all_atoms.select(main_indices)

        if len(main_atoms) == 0:
            # No main conformer atoms found, skip scaling
            return

        max_main_occ = max(main_atoms.extract_occ())

        scale_occ = self.max_occupancy / max_main_occ

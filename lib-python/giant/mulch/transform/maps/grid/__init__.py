...
def calculate_grid_size(min_carts, max_carts, grid_spacing):
    ...
    grid_size = tuple([1 + int(np.ceil(c_size / grid_spacing)) for c_size in cart_size])
...
class MultiPointGridInverter(GridInverter):
    ...
    def __call__(self, grid_indices):
        ...
        return np.transpose(np.array([i1, i2, i3]))
...
class MapGrid(object):
    ...
    def cart_points(self, origin_shift=True):
        sites = self.grid_points().astype(float)
    ...
    def unit_cell(self):
        ...
        parameters = tuple(self.cart_size()) + (90.0, 90.0, 90.0)
...
class GetWarpedMapGrid(object):
    ...
    def initialise_mask_dataset(
        ...
        if mask_dataset is None:
            ...
            if reference_dataset is None:
                raise ValueError(
                    "Must provide reference_dataset if align_mask_to_reference is True"
                )
    ...
    def mask_grid(
        ...
        ref_sites_cart = main_h.atoms().extract_xyz()

        sym_sites_cart = sym_h.atoms().extract_xyz()
        ...
        ), "inner": make_inner_mask(
            sites_cart=ref_sites_cart,
        ), "symmetry": make_symmetry_mask(
    ...
    def partition_grid(
        ...
        from .partition import MakeVoronoiGridPartition

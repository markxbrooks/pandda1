...
class CifManager(object):
    ...
    def __str__(self):
        ...
        block_string = "\n".join([b.name for b in self])

        list_block_string = "\n".join(
            [
                "Block {block_name}\n\t{block_string}".format(
                    block_name=b.name,
                    block_string=str(b).replace("\n", "\n\t"),
                )
                for b in self
        ...
        s_ = (
            ...
            list_block_string=(list_block_string).strip("\n").replace("\n", "\n|\t"),
        )
    ...
    def from_file(cls, filepath):
        ...
        cif_obj = gm.cif.read(str(filepath))

        return cls(cif_obj=cif_obj)

    def write_cif(self, filepath):

        self.cif_obj.write_file(str(filepath))
    ...
    def get_unique_column_values(
        ...
        block = self.cif_obj.find_block(block_name)
    ...
    def replace_matching_values_by_column_name(
        self,
        column_name,
        old_value,
        new_value,
    ...
    def replace_matching_values_by_column_suffix(
        self,
        column_suffix,
        old_value,
        new_value,
    ...
    def print_references_to(self, column=None, value=None):
        ...
        for block in self.cif_obj:
            ...
            for item in block:
                ...
                if (column is not None) and (column in item.loop.tags):

                    logger(
                        "column {} is in block {}".format(
                            column,
                            block.name,
                        )
                    )

                if (value is not None) and (value in item.loop.values):
                    ...
                    for r in t:
                        ...
                        if value in vals:
                            ...
                            logger(
                                "value {} is in column {} in block {}".format(
                                    value,
                                    t.column(i).tag,
                                    block.name,
                                )
    ...
    def get_link_id_for_linking_atoms(
        self,
        atom_1_resname,
        atom_1_name,
        atom_2_resname,
    ...
    def get_linking_atoms(self, link_id):
        ...
        list_block = self.cif_obj.find_block(self.LINK_LIST_NAME)
        ...
        link_block = self.cif_obj.find_block(self.LINK_BLOCK_PRE + link_id)
...
class CifIDGenerator(object):
    ...
    def increment_link(self, name):
        ...
        new_lab = (
            first[0:2] + "-" + second[0:2] + "-02"  # because the original is the first
        )
    ...
    def increment_link_2(self, name):
        ...
        new_lab = first + "-" + second + "-{:02d}".format(i + 1)
        ...
        for i_cif, other_cif in enumerate(cif_managers[1:]):
            ...
            for other_block in other_cif.cif_obj:
                ...
                if main_block is None:
                    ...
                    main_cif.cif_obj.add_copied_block(other_block)
    ...
    def merge_blocks(main_block, other_block):
        ...
        for other_item in other_block:
            ...
            if other_loop is not None:
                ...
                # Get matching column
                main_loop_col = main_block.find_loop(other_loop.tags[0])

                if main_loop_col is None:

                    # new loop
                    main_block.add_item(other_item)
                    ...
                    for other_row in other_block.item_as_table(other_item):
                        ...
                        main_loop.add_row(list(other_row))

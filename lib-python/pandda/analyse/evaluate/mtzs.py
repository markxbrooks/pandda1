...
class ConvertMapsToMtz(object):
    ...
    def __call__(
        ...
        mtz_obj.set_data(np.concatenate(all_mtz_data, axis=1))
    ...
    def prepare_mtz_data(
        ...
        mtz_obj.add_column("F{}".format(map_label), "F")
        mtz_obj.add_column(
            "PH{}".format(map_label),
            "P",
        )
        ...
        out_data.append(
            np.absolute(map_sfs.value_array).reshape((map_sfs.value_array.size, 1))
        )
        out_data.append(
            np.angle(
                map_sfs.value_array,
                deg=True,
            ).reshape((map_sfs.value_array.size, 1))
        )
...
class MakePanddaMainMtz(object):
    ...
    def __call__(
        ...
        filepath = self.output_directory / self.output_filename_template.format(
            dkey=dataset_key,
        )
    ...
    def get_map_paths_and_labels(self, dataset_files):
        ...
        if "dataset_map" in dataset_files:
            ...
            map_paths.append(dataset_files["dataset_map"])
            map_labels.append("DATASET")

        if "ground_map" in dataset_files:

            map_paths.append(dataset_files["ground_map"])
            map_labels.append("GROUND")

        if "z_map" in dataset_files:

            map_paths.append(dataset_files["z_map"])
            map_labels.append("ZVALUES")
...
class MakePanddaEventMtz(object):
    ...
    def make_event_map(
        ...
        event_map_values = np.array(dataset_map.grid) - (
            event_bdc * np.array(ground_map.grid)
        )
        ...
        dataset_map.write_ccp4_map(str(filepath))
...
class MakePanddaEvaluationMtzs(object):
    ...
    def __call__(
        ...
        **kwargs
        ...
        ).get(self.make_maps.output_key, None)
        ...
        if len(job_list) > 0:
            ...
            for dkey, of in zip(job_keys, results):
                ...
                merge_dicts(
                    ...
                    merge_dict={
                        dkey: of
                    },  # every sub-dict "of" is required to have a unique structure
                )
    ...
    def get_main_job(
        ...
        make_main_mtz = MakePanddaMainMtz(
            ...
            output_directory=(self.dataset_dir / dataset_key / self.dataset_subdir),
        )
    ...
    def get_event_jobs(
        ...
        jobs = [
            ...
            for event_num, event_bdc in dataset_event_bdcs.items()
        ]

        return jobs

    def get_wrapper(self, function, *args, **kwargs):

        from giant.processors import ProcessWrapper

        return ProcessWrapper(func=function, *args, **kwargs)

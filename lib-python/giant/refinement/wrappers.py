...
class _refiner(object):
    ...
    def __init__(self, pdb_file, mtz_file=None, cif_files=None, out_prefix=None):
        ...
        self.input_files = dict(
            pdb=(str(pdb_file) if pdb_file is not None else None),
            mtz=(str(mtz_file) if mtz_file is not None else None),
            cif=(list(map(str, cif_files)) if cif_files is not None else None),
        )

        # Eventual output prefix and output wrappers
        ...
        if os.path.exists(self.output_files["pdb"]):
            raise Sorry(
                "Output file already exists: {}".format(self.output_files["pdb"])
            )
        if os.path.exists(self.output_files["mtz"]):
            raise Sorry(
                "Output file already exists: {}".format(self.output_files["mtz"])
            )
    ...
    def run(self):
        """...run refinement amd export wrappers"""
        result = self.refine()
    ...
    def export(self):
        """Copy wrappers to output destination"""
        ...
        if not tmp_pdbs:
            raise RefinementError("No refined wrappers found: {}".format(self.tmp_dir))
        tmp_pdb = tmp_pdbs[0]
        ...
        if not (
            os.path.exists(self.output_files["pdb"])
            and os.path.exists(self.output_files["mtz"])
        ):
            raise RefinementError(
                "Output PDB & MTZ wrappers do not exist: {} / {}".format(
                    self.output_files["pdb"],
                    self.output_files["mtz"],
                )
            )
...
class refine_phenix(_refiner):
    ...
    def add_parameter_file(self, parameter_file):
        ...
        if not (
            parameter_file.endswith(".eff")
            or parameter_file.endswith(".def")
            or parameter_file.endswith(".params")
        ):
            raise Sorry("Parameter wrappers must end with .eff or .def or .params")
    ...
    def set_n_cycles(self, n):
        self.dispatcher.append_arg("main.number_of_macro_cycles={:d}".format(n))
        return self

    def set_refine_coordinates_only(self):
        self.dispatcher.append_arg("refine.strategy=individual_sites")
        return self

    def set_refine_b_factors_only(self):
        self.dispatcher.append_arg("refine.strategy=individual_adp")
        return self
...
class refine_refmac(_refiner):
    ...
    def setup(self):
        ...
        if (cif_files is not None) and (len(cif_files) > 0):
            ...
            if len(cif_files) > 1:
                ...
                cif_file = (
                    os.path.splitext(self.output_files["pdb"])[0]
                    + "-combined-restraints.cif"
                )
            ...
            self.dispatcher.extend_args(["LIBIN", cif_file])
    ...
    def add_parameter_file(self, parameter_file):
        with open(parameter_file, "r") as fh:
            for line in fh.readlines():
                if line.strip() == "END":
                    logger(
                        "END line found in parameter file -- ignoring lines from this point on"
                    )
                    break
    ...
    def set_n_cycles(self, n):
        self.dispatcher.append_stdin("NCYC {:d}".format(n))
        return self

    def set_refine_coordinates_only(self):
        self.dispatcher.append_stdin("BREF OVER")
        return self

    def set_refine_b_factors_only(self):
        self.dispatcher.append_stdin("REFI BONLY")
        return self
    ...
    def complete_model(self):
        self.dispatcher.append_stdin("BUIL Y")
        return self
...
def get_refiner(name):
    ...
    try:
        ...
        raise Failure(
            "Invalid refinement program selected: {} (options {})".format(
                name, list(refiners.keys())
            )
        )
    raise Failure(
        "Invalid refinement program selected: {} (options {})".format(

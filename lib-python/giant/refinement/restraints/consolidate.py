...
from .links import LinkRecordsToPhenixParams, UpdateLinksFromCif
...
class ConsolidationResult(object):
    ...
    def write_phenix_params(self, path):

        logger("Writing {}".format(str(path)))

        make_params = LinkRecordsToPhenixParams()

        params_str = make_params(
...
class ConsolidateRestraintsAndUpdateModel(object):
    ...
    def __call__(
        ...
        logger.subheading("Consolidated Cif Object")

        logger(str(merged_cif_obj))
        ...
        if model is None:
            ...
            logger.subheading("Updating link records")
            ...
            logger.subheading("Updated link records from model")

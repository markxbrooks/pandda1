...
from giant.structure.b_factors import CalculateSurroundingsBFactorRatio
from giant.structure.common import GetInterestingResnames
from giant.structure.formatting import short_labeller
from giant.structure.occupancy import CalculateResidueGroupOccupancy
from giant.structure.rmsds import CalculatePairedConformerSetsRMSDs

from .edstats import CalculateEdstatsScores
from .plots import ValidationDistributionPlots, ValidationRadarPlot
...
class WriteScores(object):
    ...
    def __init__(
        ...
        self.make_radar_plot = (
            make_radar_plot if make_radar_plot is not None else ValidationRadarPlot()
        )
    ...
    def __call__(
        ...
        path_dict.update(
            self.write_residue_scores_csv(
                ...
                out_path=(dir_path / "residue_scores.csv"),
            )
        )
        ...
        path_dict.update(
            self.make_residue_scores_radar_plots(
                residue_scores_df=residue_scores_df,
                out_prefix=(dir_path / "residue_scores_"),
            )
        )
        ...
        path_dict.update(
            self.make_residue_scores_distributions(
                residue_scores_df=residue_scores_df,
                out_path=(dir_path / "score_distributions.png"),
            )
        )

        logger(self.format_dict(path_dict))
    ...
    def write_residue_scores_csv(self, residue_scores_df, out_path):
        ...
        residue_scores_df.to_csv(str(out_path))
    ...
    def make_residue_scores_radar_plots(self, residue_scores_df, out_prefix):
        ...
        for index, values in residue_scores_df.iterrows():
            ...
            img_path = str(out_prefix) + label + ".png"
...
class ScoreModelMultiple(object):
    ...
    def __init__(
        ...
        self.score_model = score_model if score_model is not None else ScoreModel()

        self.processor = processor if processor is not None else basic_processor
    ...
    def __call__(
        ...
        if reference_datasets is None:
            reference_datasets = [None] * len(dataset_list)
        elif len(reference_datasets) == 1:
            reference_datasets = reference_datasets * len(dataset_list)
        else:
            assert len(dataset_list) == len(reference_datasets)

        labels = [d.tag for d in dataset_list]

        assert len(labels) == len(set(labels)), "datasets must have unique .tags"
    ...
    def concatenate_tables(self, tables, labels):
        ...
        TABLE_COLUMNS = self.TABLE_COLUMNS + self.score_model.TABLE_COLUMNS
...
class ScoreModelSingle(object):
    ...
    def __init__(
        ...
        self.calculate_electron_density_scores = CalculateEdstatsScores(
            parse_logfile=False
        )

        self.calculate_surroundings_b_factor_ratio = CalculateSurroundingsBFactorRatio()

        self.calculate_residue_group_occupancy = CalculateResidueGroupOccupancy()

        self.calculate_paired_conformer_sets_rmsds = CalculatePairedConformerSetsRMSDs()
    ...
    def score_dataset(self, dataset):
        ...
        ed_scores = (
            ...
            if (
                ...
                and (dataset.model.filename is not None)
                and (dataset.data is not None)
                and (dataset.data.filename is not None)
            )
            else None
        )
        ...
        for rg in residue_groups:
            ...
            if ed_scores is not None:
                ...
                if rg_occ > 0.0:
                    rg_dict["RSZO/OCC"] = rg_scores.rszo / rg_occ
            ...
            scores_dict[rg_label] = rg_dict

        return scores_dict

    def calculate_intra_model_rmsds(
        self,
        dataset_1,
        dataset_2,
    ):
        ...
        rgs_2_dict = {short_labeller(rg): rg for rg in rgs_2}
    ...
    def _integrate_reference_dict(
        ...
        for r_key, r_dict in reference_dict.items():
            ...
            for kk in self.ROOT_COLUMNS:
                ...
                try:
                    d_dict["d" + kk] = d_dict[kk] - r_dict[kk]
                except:
                    pass

    def _integrate_rmsd_dict(
        self,
        dataset_dict,
        rmsd_dict,
    ):

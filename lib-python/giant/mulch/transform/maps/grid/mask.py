...
class GridMask(object):
    ...
    def __str__(self):
        ...
        s_ = (
            ...
            mask_frac=(float(self.mask_size) / float(np.prod(self.grid_size))),
        )
    ...
    def get_mask_binary(self):
        ...
        mask_binary = np.zeros(np.prod(self.grid_size), dtype=bool)
    ...
    def embed_data(self, data, masked_value=0.0):
        ...
        embedded = masked_value * np.ones(np.prod(self.grid_size))
...
class GetNonPeriodicSitesMask(object):
    ...
    def filter_sites(self, sites_cart):
        ...
        buffer_grid = self.unit_cell.fractionalize(flex.vec3_double([buffer_cart]))[0]
    ...
    def mask_sites(self, sites_cart):
        ...
        n_unit_cells = tuple(1 + int(np.ceil(buffer_g)) for buffer_g in buffer_grid)
        ...
        expanded_unit_cell = unit_cell(
            tuple(
                n * p
                for n, p in zip(
                    tuple(n_unit_cells) + (1.0, 1.0, 1.0), self.unit_cell.parameters()
                )
            )
        )
        ...
        expanded_grid_mask = np.zeros(np.prod(expanded_grid_size), dtype=bool)
        ...
        grid_mask = expanded_grid_mask[
            0 : self.grid_size[0], 0 : self.grid_size[1], 0 : self.grid_size[2]
        ]
...
def compound_grid_masks(
    ...
    total_binary = np.zeros(np.prod(grid_size), dtype=bool)

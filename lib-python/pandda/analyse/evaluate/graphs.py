...
from pandda.graphs import PanddaDatasetPlotter, PanddaMultiDatasetPlotter
...
class MapDistributionPlotter(PanddaDatasetPlotter):
    ...
    def plot(
        self, dataset_label, datasets_map_dict, statistical_model, *args, **kwargs
    ):

        difference_map_values = (
            datasets_map_dict.get(dataset_label) - statistical_model.mu_map_data
        )
...
class QQDistributionPlotter(PanddaDatasetPlotter):
    ...
    def make_qq_plot(
        ...
        p_values = np.linspace(0.0, 1.0, len(difference_map_values) + 2)
        e_values = st.norm.ppf(p_values, 0.0, 1.0)[1:-1]  # chop ends which are inf
    ...
    def __init__(
        ...
        self.plotters = [
            MapDistributionPlotter(
                output_path_template=str(d_dir / "{label}-distribution.png"),
            ),
            MapScatterPlotter(
                output_path_template=str(d_dir / "{label}-scatter.png"),
            ),
            QQDistributionPlotter(
                output_path_template=str(d_dir / "{label}-qqplot.png"),
            ),
        ]

        self.processor = processor

    def __call__(self, datasets, datasets_map_dict, statistical_model, *args, **kwargs):

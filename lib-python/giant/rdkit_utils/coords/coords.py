...
from bamboo.rdkit_utils.coords.utils import (
    calculate_coordinate_differences,
    get_centroid_from_file,
)
from rdkit import Chem


def calculate_fraction_of_atom_pairs_within_tolerance(
    model1, model2, tolerance=1, suppress=False
):
    """Calculate the percentage of atoms in model1 that are within `tolerance` of the corresponding atom in model2"""

    try:
        differences = calculate_coordinate_differences(model1, model2)
    ...
    if not differences:
        if suppress:
            ...
            raise Exception(
                "Error calculating coordinate differences.\n\t{!s}\n\t{!s}".format(
                    model1, model2
                )
            )
    ...
    for diffset in differences:
        within = sum(
            [1.0 for d in diffset if (d[0] ** 2 + d[1] ** 2 + d[2] ** 2 < tolerance**2)]
        )
...
def calculate_fraction_of_atom_pairs_within_tolerance_using_symmetry(
    model1, model2, tolerance=1, suppress=False, delete_structures=True
):
    ...
    tempfile = map_to_reference_using_symmetry(
        refpdb=model1, movpdb=model2, pdbout=model1.replace(".pdb", ".temp")
    )
    # Calculate the RMSD as normal
    percentage = calculate_fraction_of_atom_pairs_within_tolerance(
        model1, tempfile, tolerance, suppress
    )
...
def calculate_rmsd_using_symmetry(
    model1, model2, suppress=False, delete_structures=True
):
...
def calculate_centroid_difference_using_symmetry(
    model1, model2, delete_structures=True
):

...
class OccupancyGroup(object):
    ...
    def __str__(self):
        ...
        formatted_string = "\n".join(
            [str(Labeller.format(o)).strip() for o in self.objects]
        )
        ...
        s_ = (
            ...
            name=str(self.label),
            object_string=str(formatted_string).strip("\n").replace("\n", "\n|\t"),
        )
...
class OccupancyRestraint(object):
    ...
    def __str__(self):
        ...
        formatted_groups = "\n".join([str(g).strip() for g in self.occupancy_groups])
        ...
        s_ = (
            ...
            groups=str(formatted_groups).strip("\n").replace("\n", "\n|\t"),
        )
    ...
    @classmethod
    def from_list_of_lists(cls, objects_list, *args, **kwargs):
...
class OccupancyRestraintList(object):
    ...
    def add(self, occupancy_restraints):
        ...
        self.occupancy_restraints.extend(occupancy_restraints)

...
class PossibleStructureFactorChecker(DatasetChecker):
    ...
    def __call__(self, dataset):
        ...
        if len(dataset_sfs) > 1:
            messages.append(
                "Dataset contains more than one of the possible structure factors"
            )
        elif len(dataset_sfs) == 0:
            messages.append(
                "Dataset does not contain any of the possible structure factors"
            )
...
class RequiredStructureFactorChecker(DatasetChecker):
    ...
    def __call__(self, dataset):
        ...
        for miss_pair in missing_sfs:
            messages.append(
                "Dataset is missing structures factors: {}".format(str(miss_pair))
            )
        return messages
...
class CrystallographicDatasetChecker(DatasetCheckerGroup):
    ...
    def __init__(self):
        ...
        self.checks.append(CrystallographicDatasetModelChecker())
...
class DiffractionArrayChecker(DatasetCheckerGroup):
    ...
    def __init__(
        ...
        assert (possible_structure_factor_pairs is not None) or (
            required_structure_factor_pairs is not None
        )
    ...
    def __call__(self, mcd):
        ...
        if len(self.messages) > 0:
            ...
            if self.action == "raise":
                raise DatasetCheckingFailure(self.format_messages(self.messages))
            elif self.action == "warn":
                logger.warning(self.format_messages(self.messages))
            else:
                pass

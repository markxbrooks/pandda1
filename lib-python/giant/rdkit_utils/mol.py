...
return_codes = {
    0: "Reference {!s} matches the query {!s} as far as I can tell!",
...
def check_pdb_readable(file):
    ...
    if not mol:
        raise LigandCheckError("File not rdkit-readable: {!s}".format(file))
    else:
        return mol
...
def check_ligands_from_file_identical(ref, query, dynamicBondAssignment=True):
    """Check two ligands (pdb wrappers) are identical - gives similarity relative to the first file"""
    rc = check_ligand_mols_identical(
        check_pdb_readable(ref),
        check_pdb_readable(query),
        dynamicBondAssignment=dynamicBondAssignment,
    )
    return rc, return_codes[rc].format("file", "file")


def check_ligand_from_file_identical_to_smile(
    smile, query, dynamicBondAssignment=False
):
    """Checks ligand (pdb file) is the same as the smile"""
    rc = check_ligand_mols_identical(
        check_smile_readable(smile),
        check_pdb_readable(query),
        dynamicBondAssignment=dynamicBondAssignment,
    )
...
def check_ligand_identical_to_smile(smile, mol, dynamicBondAssignment=False):
    ...
    rc = check_ligand_mols_identical(
        ref=check_smile_readable(smile),
        mol=mol,
        dynamicBondAssignment=dynamicBondAssignment,
    )

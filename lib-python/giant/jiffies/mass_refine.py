...
def set_get_log(params):
    ...
    if params.log is not None:
        ...
        params.log = str(pl.Path(params.work_folder).with_suffix(".log"))
...
class MassRefineProtocol(object):
    ...
    def __init__(self, params):
        ...
        if params.mode == "configure":
            ...
            self.import_model_args = {
                "model": (pl.Path(params.refine_model.pdb)),
                "cif_restraints": (params.refine_model.cif_restraints),
                "refinement_parameters": (params.refine_model.refinement_parameters),
            }

        else:
            raise Exception("mode not recognised: {}".format(params.mode))

    def run_setup(self, params):

        from giant.mass_refine import MassRefineConfiguration, MassRefineFileSystem
    ...
    def run(self):
        ...
        if self.import_data is not None:
            ...
            new_datasets = self.import_data.run(
                data_paths=(self.import_data_args["data"]),
            )
        ...
        if self.refine_model is not None:
            ...
            model_folder = self.import_model.run(
                model_path=(self.import_model_args["model"]),
                refinement_parameters_paths=(
                    self.import_model_args["refinement_parameters"]
                ),
                cif_restraints_paths=(self.import_model_args["cif_restraints"]),
            )
    ...
    def get_update_config(
        ...
        from giant.mass_refine.processing import UpdateConfiguration
    ...
    def get_import_data(
        ...
        get_meta = (
            GetMetaFromTable(p.meta_csv_path)
            if (p.meta_csv_path is not None)
            else (
                GetMetaFromRegex(p.meta_mtz_regex)
                if (p.meta_mtz_regex is not None)
                else None
            )
        )

        import_data = ImportDatasets(
            file_system=fs,
            get_meta=get_meta,
    ...
    def get_import_model(
        ...
        from giant.mass_refine.import_model import ImportModel
        from giant.mulch.labelling import PathLabeller
    ...
    def get_refine_model(
        ...
        from giant.mass_refine.refine_model import RefineModel

...
from giant.mulch.selectors import DatasetKeySelector
from giant.utils import pretty_format_list
...
class UpdateablePartitioner(object):
    ...
    def remove(self, partition, values):
        ...
        if partition not in self.sets:
            raise ValueError('partition "{p}" does not exist'.format(partition))
    ...
    def as_dict(self):
        ...
        return {
            p_key: (list(p_set) if (p_set is not None) else [])
...
class DatasetKeyPartitioner(UpdateablePartitioner):
    ...
    def __init__(self, **kw_args):
        ...
        self.sets = {
            p_key: set(dataset_keys) for p_key, dataset_keys in kw_args.items()
        }
    ...
    def get_train_datasets(self, datasets):
        ...
        if self.sets["not_train"] is not None:
            datasets = {
                ...
                if (dtag not in self.sets["not_train"])
            }

        if self.sets["train"] is not None:
            datasets = {
                dtag: d for dtag, d in datasets.items() if (dtag in self.sets["train"])
            }
    ...
    def get_test_datasets(self, datasets):
        ...
        if self.sets["not_test"] is not None:
            datasets = {
                dtag: d
                for dtag, d in datasets.items()
                if (dtag not in self.sets["not_test"])
            }

        if self.sets["test"] is not None:
            datasets = {
                dtag: d for dtag, d in datasets.items() if (dtag in self.sets["test"])
            }
...
class ResolutionShellTestTrainPartitioner(object):
    ...
    def __str__(self):
        ...
        s_ = (
            ...
            name=self.name,
            parameters=("\n".join(parameter_strings).strip().replace("\n", "\n|\t")),
            partitioner=(str(test_train_partitioner).strip().replace("\n", "\n|\t")),
        )
    ...
    def __call__(self, datasets):
        ...
        while r_inner <= max_resolution:
            ...
            # Add to output dict
            output_shells.append((shell_bounds, dataset_partitions))

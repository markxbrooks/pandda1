...
def assign_column_labels(col_labs, col_types):
    ...
    # Find the I cols
    lab_dict["i_labels"] = [s for s in inty if (s.startswith("I"))]
    # Find the SIGI cols
    lab_dict["sigi_labels"] = [s for s in sdev if (s.startswith("SIGI"))]
    # Find the F_calc
    ...
    # Find the RFree Flag
    lab_dict["r_free_labels"] = [r for r in ints if ("FREE" in r.upper())]
    # Figure-o-merits
    lab_dict["fom_labels"] = [l for l in foms if ("FOM" in l.upper())]

    used_labels = []
    [used_labels.extend(vals) for vals in list(lab_dict.values())]

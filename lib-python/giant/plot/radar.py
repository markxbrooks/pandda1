...
class Radar(object):
    ...
    def __init__(self, titles, rect=None, fig=None):
        ...
        self.angles = [
            ...
            for a in numpy.arange(90, 90 + 360, 360.0 / self.n)
        ]
        ...
        self.axes = [
            self.fig.add_axes(rect, projection="polar", label="axes%d" % i)
        ...
        for i, (t, ang) in enumerate(
            zip(titles, numpy.deg2rad(numpy.r_[self.angles, self.angles[0]]))
        ):
            ha, va = self._ha_va(i)
            self.ax.text(
                x=ang,
    ...
    def _default_limits(self):
        ...
        for d in numpy.array(self._data).T:
            if (d == None).any():
                limits.append((0.49, 0.51))
            elif len(d) == 1:
                limits.append((d[0] - 0.01, d[0] + 0.01))
            elif min(d) == max(d):
                limits.append((d[0] - 0.01, d[0] + 0.01))
            else:
    ...
    def _calculate_scales(self):
        ...
        self._offset = [
            -min * sca for (min, max), sca in zip(self._limits, self._scales)
        ]
    ...
    def _filter_values_and_labels(
        self, values, labels, filter_large=True, filter_small=True
    ):
        ...
        for vals, labs in zip(values, labels):
            ...
            for v, l in zip(vals, labs):
                if (v > filt_lower or not filter_small) and (
                    v < filt_upper or not filter_large
                ):
                    vs.append(v)
                    ls.append(l)
                elif v >= filt_upper:
                    vs.append(filt_upper)
                    ls.append(l)
                elif v <= filt_lower:
                    vs.append(filt_lower)
                    ls.append(l)
            filt_vals.append(vs)
            filt_labs.append(ls)
    ...
    def _ha_va(self, i_axis, invert=False):
        ...
        ha = (
            has[0]
            if ((i_frac <= 0.1) or (i_frac >= 0.9) or (0.4 <= i_frac <= 0.6))
            else has[1] if ((0.1 < i_frac < 0.4)) else has[2]
        )

        va = (
            vas[0]
            if ((0.2 <= i_frac <= 0.3) or (0.7 <= i_frac <= 0.8))
            else vas[1] if ((i_frac < 0.2) or (i_frac > 0.8)) else vas[2]
        )
        return ha, va
    ...
    def _apply_ticks(self):
        ...
        tick_vals, tick_labs = self._filter_values_and_labels(
            tick_vals, tick_labs, filter_large=True, filter_small=True
        )
    ...
    def plot(self):
        ...
        norm_vals = numpy.array(
            self._normalise(numpy.array(self._data).T.tolist())
        ).T.tolist()

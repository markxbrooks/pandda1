...
DESCRIPTION = """
    ...
    2) Defining custom names for the output folder and the output wrappers
...
def copy_input_files_to_output_folder(out_dir, params):
    ...
    logger.subheading("Copying/linking wrappers to refinement folder")

    # Output filenames for "input" wrappers
...
def run(params):
    ...
    out_dir, previous_dirs = get_new_output_directory(
        dir_prefix=params.output.dir_prefix
    )
    ...
    if previous_dirs:
        logger(
            "Found existing refinement directories: \n\t{}\n".format(
                "\n\t".join(previous_dirs)
            )
        )
    ...
    logger.subheading("Post-processing output wrappers")

...
input {
    ...
    labelling = *filename foldername
        ...
        .help = "how should a label be extracted from the input file(s)? Used for naming the output wrappers."
}
output {
    out_dir = .
        .type = path
...
def run(params):
    ...
    from giant.paths import easy_directory, filename, foldername
    ...
    logger(
        ps(
            "Aligning {} structure(s) to {}".format(
    ...
    for p_mov in params.input.align_pdb:
        ...
        for ali_label, ali_sel in zip(align_labels, align_selections):
            ...
            try:
                ...
                logger.warning(
                    ps("Could not align selection {}: {}".format(ali_sel, str(e))).red()
                )
                continue

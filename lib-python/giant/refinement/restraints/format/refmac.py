...
from giant.structure.formatting import RefmacSelection
        ...
        return "\n".join(
            [
                ...
                self._group_string_2.format(
                    complete=("complete" if (complete is True) else "incomplete"),
...
class RefmacRestraintCollectionFormatter(object):
    ...
    def __init__(self):
        ...
        self.format_distance_restraint = RefmacDistanceRestraintFormatter()

        self.format_occupancy_restraint = RefmacOccupancyRestraintFormatter()

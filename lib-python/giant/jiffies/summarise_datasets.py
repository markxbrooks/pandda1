...
DESCRIPTION = """
    A tool to summarise the variation in sets of pdb and/or mtz wrappers.
"""
...
def validate_labels(labels, filenames):
    ...
    logger(
        "Input filenames/labels: \n\t{}".format(
            "\n\t".join(["{} : {}".format(l, f) for l, f in zip(labels, filenames)]),
        )
    )
    ...
    if len(labels) != len(set(labels)):
        ...
        raise Sorry(
            "One or more labels are the same for the input wrappers: \n\t{}\n\tConsider using a different labelling function".format(
...
def format_row(cols, row="", col_width=20, header=False):
    if header:
        ...
        fmt = ".3f"
    s = (
        ("| " + format_column(row, col_width, "", "")) * bool(row)
        + "|"
        + "|".join([format_column(c, col_width, "^", fmt) for c in cols])
        + "|"
    )
    if header:
        s = (
...
def crystal_min_max(title, crystals, label_hash, value_func):
    ...
    s_out.append(
        "Smallest {}: {} - {} / {}".format(
            title, value_func(smallest), smallest.name, label_hash[smallest.name]
        )
    )
    s_out.append(
        "Largest {}:  {} - {} / {}".format(
            title, value_func(largest), largest.name, label_hash[largest.name]
        )
    )
    return "\n".join(s_out)
...
def run(params):
    ...
    if params.input.pdb:
        ...
        logger.heading("Grouping {} pdb wrappers by space group".format(n))
        ...
        for cg in crystal_groups:
            ...
            logger(
                crystal_statistics(
                    "Resolution",
                    cg.crystals,
                    value_func=lambda c: c.resolution_high,
                    header=True,
                )
            )
            ...
            logger(
                crystal_statistics(
                    "R-work", cg.crystals, value_func=lambda c: c.r_work, header=True
                )
            )
            logger(
                crystal_statistics(
                    "R-free", cg.crystals, value_func=lambda c: c.r_free, footer=True
                )
            )
    ...
    if params.input.mtz:
        ...
        logger.heading("Grouping {} mtz wrappers by space group".format(n))
        ...
        for cg in crystal_groups:
            ...
            for c in cg.crystals:
                for label in params.options.column_label:
                    ...
                    if label not in c.column_labels:
                        logger(
                            'Checking: column "{}" not in diffraction data of {}. columns present are {}'.format(
                                label, label_hash[c.name], c.column_labels
                            )
                        )
                        error = True
            if error is True:
            ...
            logger(
                crystal_statistics(
                    "Resolution (low)",
                    cg.crystals,
                    value_func=lambda c: c.resolution_low,
                )
            )
            logger(
                crystal_statistics(
                    "Unit cell - vol",
                    cg.crystals,
                    value_func=lambda c: c.unit_cell.volume(),
                    header=True,
                )
            )
            logger(
                crystal_statistics(
                    "Unit cell - a",
                    cg.crystals,
                    value_func=lambda c: c.unit_cell.parameters()[0],
                )
            )
            logger(
                crystal_statistics(
                    "Unit cell - b",
                    cg.crystals,
                    value_func=lambda c: c.unit_cell.parameters()[1],
                )
            )
            logger(
                crystal_statistics(
                    "Unit cell - c",
                    cg.crystals,
                    value_func=lambda c: c.unit_cell.parameters()[2],
                )
            )
            logger(
                crystal_statistics(
                    "Unit cell - alpha",
                    cg.crystals,
                    value_func=lambda c: c.unit_cell.parameters()[3],
                )
            )
            logger(
                crystal_statistics(
                    "Unit cell - beta",
                    cg.crystals,
                    value_func=lambda c: c.unit_cell.parameters()[4],
                )
            )
            logger(
                crystal_statistics(
                    "Unit cell - gamma",
                    cg.crystals,
                    value_func=lambda c: c.unit_cell.parameters()[5],
                )
            )

            wavelength_func = (
                lambda c: iotbx.mtz.object(label_hash[c.name])
                .crystals()[1]
                .datasets()[0]
                .wavelength()
            )

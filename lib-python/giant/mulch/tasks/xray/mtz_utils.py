...
from giant.mulch.dataset import CrystallographicData
...
class RemoveColumns(object):
    ...
    def __call__(
        ...
        keep_columns = [
            ...
            if col not in (["H", "K", "L"] + self.columns_to_remove)
        ]

        keep_comms = [
            "E{i}={col}".format(i=i + 1, col=col) for i, col in enumerate(keep_columns)
        ]
        ...
        if not out_mtz.exists():
            ...
            prog.write_output(str(out_mtz.with_suffix(".log")))
...
class FillMissingReflections(object):
    ...
    def __call__(
        ...
        # Stage 3 - remerge the two wrappers
    ...
    def get_fill_limits(
        ...
        m = CrystallographicData.from_file(str(in_mtz))
...
class PrepareMTZTask(object):
    ...
    def __init__(
        ...
        self.reindex_to_reference = reindex_to_reference

        self.fill_missing_reflections = fill_missing_reflections

        self.transfer_rfree_flags = transfer_rfree_flags
    ...
    def run(self, in_mtz):
        ...
        # List of intermediate wrappers to be deleted at the end
        intermediate_files = []
    ...
    def copy_to_directory(self, filepath, directory):
        ...
        out_path = directory / filepath.name

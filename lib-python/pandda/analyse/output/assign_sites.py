...
class AssignEventsToSites(object):
    ...
    def group_events_into_sites(self, events):
        ...
        centroids = [e["xyz_centroid_ref"] for e in events]
        ...
        # List of sites
        sorted_sites = [sites[i] for i in site_list_sort_hash]

        # Map events to sites
        event_site_idxs = [site_list_sort_hash.index(i) for i in event_site_idxs]
    ...
    def make_site(self, events):
        ...
        site = collections.OrderedDict(
            [
                ...
                (
                    "max_value",
                    np.max([e["z_peak"] for e in events]),
                ),
                (
                    "xyz_centroid",
                    tuple(
                        round(v, 3)
                        for v in np.mean(
                            [e["xyz_centroid_ref"] for e in events], axis=0
                        )
                    ),
                ),
                (

...
# Optional crystallography stack (cctbx); skip tests if missing
mmtbx = importorskip("mmtbx")
import mmtbx.alignment


def align_sequences_default(seq_a, seq_b):
    """Align two sequences using a default setting"""
    if isinstance(seq_a, list):
        seq_a = "".join(seq_a)
    if isinstance(seq_b, list):
        seq_b = "".join(seq_b)
...
def pairwise_sequence_identity(
    seqs_1, seqs_2, min_alignment=0.90, seq_identity_threshold=None
):
    """
    ...
    if isinstance(min_alignment, float):
        assert (
            0.0 <= min_alignment <= 1.0
        ), "min_alignment must be either an integer or between 0.0-1.0 or None"
        min_ali_frac = min_alignment
    elif isinstance(min_alignment, int):
        min_ali_num = min_alignment
    else:
        assert (
            min_alignment is None
        ), "min_alignment must be either an integer or between 0.0-1.0 or None"
...
def pairwise_chain_sequence_identity(chains_1, chains_2, seq_identity_threshold=None):
    ...
    return pairwise_sequence_identity(
        seqs_1, seqs_2, seq_identity_threshold=seq_identity_threshold
    )

...
class WriteRestraintsPymolScript(object):
    ...
    def initialise(self):
        ...
        if self.base_python_script is not None:
            python_script.lines.extend(copy.deepcopy(self.base_python_script.lines))
    ...
    def make_distance_restraint_objects(
        self, python_script, restraints_collection, hierarchy
    ):
        ...
        for r in restraints_collection.distance_restraints:
            ...
            atom1 = hierarchy.select(
                h_cache.selection(PhenixSelection.format(r.atom1))
            ).only_atom()
            atom2 = hierarchy.select(
                h_cache.selection(PhenixSelection.format(r.atom2))
            ).only_atom()
    ...
    def make_occupancy_restraint_objects(
        self, python_script, restraints_collection, hierarchy
    ):

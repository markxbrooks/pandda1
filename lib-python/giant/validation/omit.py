...
class MakeAltlocOmitMap(object):
    ...
    def __call__(self, dataset, altlocs, dir_path=pl.Path()):
        ...
        out_base = self.output_prefix + "".join(sorted(altlocs))

        out_prefix = dir_path / out_base
    ...
    def make_omit_map(
        ...
        return (refine.output_files["pdb"], refine.output_files["mtz"])
...
class MakeLeaveOneOutOmitMaps(object):
    ...
    def __call__(self, dataset, dir_path=pl.Path()):
        ...
        unique_altloc_combinations = list(
            map(tuple, sorted(set(resname_altloc_hash.values())))
        )
        ...
        residue_result_dict = {
            ...
            for rg_label, altlocs in resname_altloc_hash.items()
        }
    ...
    def get_resname_altloc_sets(self, dataset):
        ...
        unused_conformers = all_conformers.difference(all_conformers_used)

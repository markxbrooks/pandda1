...
# Optional dependency: scitbx (part of cctbx)
importorskip("scitbx.array_family")
...
def test_LinearScaling():
    ...
    assert v0 == approx(round(float(ls.optimised_values[0]), 3))
    assert v1 == approx(round(float(ls.optimised_values[1]), 3))

    # Check scaling is working correctly
    scaled = ls.transform(t1_mov_values)

    assert 0.0 == approx(
        flex.sum(t1_ref_values - scaled),
        abs=1e-6,
    )

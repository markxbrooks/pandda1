...
from giant.xray.unit_cell import (
    UnitCellVariation,
    lcv_from_unit_cells,
    pairwise_ecv,
    pairwise_lcv,
...
class CrystalGroup(object):
    def __init__(self, crystals):
        ...
        self.space_groups = list(
            set([c.space_group.info().symbol_and_number() for c in self.crystals])
        )
    ...
    def dendrogram(self, fname, method="lcv", **kwargs):
        ...
        if method == "lcv":
            ...
            link_func = pairwise_ecv
        return unit_cell_dendrogram(
            fname=fname,
    ...
    def by_space_group(cls, crystals):
        ...
        return [
            cls(list(x[1]))
            for x in itertools.groupby(sorted(crystals, key=sg_func), key=sg_func)
        ]
    ...
    def by_unit_cell(cls, crystals, method="lcv", cutoff=0.5):
        ...
        link_mat = scipy.cluster.hierarchy.linkage(
            dist_mat_con, method="single", metric="euclidean"
        )
        clusters = scipy.cluster.hierarchy.fcluster(
            link_mat, t=cutoff, criterion="distance"
        )
        from giant.common.clustering import generate_group_idxs

        return [
            cls([crystals[idx] for idx in g])
...
def dendrogram(
    ...
    dend = scipy.cluster.hierarchy.dendrogram(
        link_mat, p=num_nodes, truncate_mode="lastp", labels=labels
    )
...
def unit_cell_dendrogram(fname, unit_cells, link_func, labels=None, **kwargs):
    """Calculate a dendrogram for a set of mtz wrappers/object, clustering by unit_cell parameters"""
...
def sort_pdbs_by_space_group(pdb_files=None, pdb_inputs=None, labels=None):
    ...
    if pdb_files:
        pdb_inputs = [None] * len(pdb_files)
    else:
        pdb_files = [None] * len(pdb_inputs)
    # Check or create labels
    if labels:
        assert len(labels) == len(
            pdb_files
        ), "labels must be the same length as pdb_files/pdb_objects"
    else:
        labels = list(range(len(pdb_files)))
    # Create summary objects
    from giant.xray.crystal import CrystalInfo

    pdb_summaries = [
        CrystalInfo.from_pdb(pdb_file=p_f, pdb_input=p_o, name=lab)
        for p_f, p_o, lab in zip(pdb_files, pdb_inputs, labels)
    ]
    # Group by SpaceGroup
...
def sort_mtzs_by_spacegroup(mtz_files=None, mtz_objects=None, labels=None):
    ...
    if mtz_files:
        mtz_objects = [None] * len(mtz_files)
    else:
        mtz_files = [None] * len(mtz_objects)
    # Check or create labels
    if labels:
        assert len(labels) == len(
            mtz_files
        ), "labels must be the same length as mtz_files/mtz_objects"
    else:
        labels = list(range(len(mtz_files)))
    # Create summary objects
    from giant.xray.crystal import CrystalInfo

    mtz_summaries = [
        CrystalInfo.from_mtz(mtz_file=m_f, mtz_object=m_o, name=lab)
        for m_f, m_o, lab in zip(mtz_files, mtz_objects, labels)
    ]
    # Group by SpaceGroup

...
class GetMetaFromTable(object):
    ...
    def __init__(self, meta_path):
        ...
        self.meta_table = pd.read_csv(str(self.meta_path))
...
class GetMetaFromRegex(object):
    ...
    def __init__(self, meta_regex):
        ...
        self.regex_string = str(meta_regex)

        self.regex_compile = re.compile(str(meta_regex))

    def __call__(self, string):

        m = self.regex_compile.search(str(string))
...
class ImportDatasets(object):
    ...
    def __init__(
        ...
        self.get_meta = get_meta if get_meta is not None else None

        self.use_meta_as_label = bool(use_meta_as_label)
    ...
    def run(
        ...
        for m in data_paths:
            ...
            if self.use_meta_as_label is True:
                ...
                label = "_".join(
                    ["{k}_{v}".format(k=k, v=v) for k, v in sorted(meta_dict.items())]
                )
            ...
            import_tuples.append((label, m, meta_dict))

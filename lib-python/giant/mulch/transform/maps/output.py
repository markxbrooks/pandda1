...
class MapGridMapWriter(object):
    ...
    def __call__(
        ...
        t_map_data = cctbx.maptbx.rotate_translate_map(
            ...
            map_data=map_data,
            rotation_matrix=scitbx.matrix.rec(
                [1, 0, 0, 0, 1, 0, 0, 0, 1], (3, 3)
            ).elems,
            translation_vector=(
                -1.0 * scitbx.matrix.rec(list(self.grid_origin), (3, 1))
            ).elems,
        )
...
class NativeMapMaker(object):
    ...
    def embed_grid_data(
        ...
        native_sites = dataset.model.alignment.ref2nat(self.sites_mask)
        ...
        sc_sample_values = reference_map.get_cart_values(
            dataset.model.alignment.nat2ref(sc_sample_points - shift_vector)
        )
    ...
    def calculate_supercell_size(self, sites_cart, dataset):
        ...
        supercell_size = tuple(
            int(np.ceil(ma - mi)) for mi, ma in zip(*box_frac_min_max)
        )

        return supercell_size

    def get_supercell_unit_cell(
        self,
        unit_cell,
        supercell_size,
    ):
    ...
    def get_supercell_gridding(
        ...
        supercell_nreal = tuple(
            int(s * n) for s, n in zip(supercell_size, unit_cell_gridding.n_real())
        )
    ...
    def get_vector_to_centre_sites_on_unit_cell(
        self,
        sites_cart,
        unit_cell,
    ):
        ...
        sites_centre = tuple((sites_cart.max(axis=0) + sites_cart.min(axis=0)) / 2.0)

        uc_centre = unit_cell.orthogonalize((0.5, 0.5, 0.5))

        shift_vector = tuple(a - b for a, b in zip(uc_centre, sites_centre))
    ...
    def map_grid_points_to_cart_points(
        self,
        sites_grid,
        gridding,
        unit_cell,
        ...
        sites_cart = np.array(list(map(g2c, map(tuple, sites_grid))))
    ...
    def roll_expand_supercell_data(
        self,
        supercell_map_data,
        supercell_size,
    ):
        ...
        n_shift = tuple(int(n / s) for n, s in zip(n_total, supercell_size))
        ...
        for xyz in flex.nested_loop(supercell_size):
            ...
            n_roll = tuple(n * int(i) for n, i in zip(n_shift, xyz))
    ...
    def extract_subcell_of_supercell(
        self,
        supercell_map_data,
        supercell_size,
    ):
        ...
        n_one = tuple(int(n / s) for n, s in zip(n_all, supercell_size))
...
class MaskedNativeMapMaker(object):
    ...
    def __init__(
        ...
        self.make_map = NativeMapMaker(
            ...
            sites_mask_radius=2.0 * map_grid.grid_spacing(),
        )

...
input {
    pdb = None
        .help = 'Input PDB wrappers - multiple can be provided'
        .type = str
        .multiple = True
}
options {
...
from giant.structure.altlocs import (  # noqa: E402
    expand_alternate_conformations,
    prune_redundant_alternate_conformations,
)


def standardise_multiconformer_model(
    hierarchy, pruning_rmsd=0.1, in_place=False, verbose=False
):
...
def run(params):
    ...
    if not params.input.pdb:
        raise IOError("No pdb wrappers have been provided")
    ...
    for pdb in params.input.pdb:
        ...
        if obj.hierarchy.models_size() > 1:
            raise Sorry(
                "Input structures may only have one model (current structure has {})".format(
                    obj.hierarchy.models_size()
                )
            )
        ...
        final.write_pdb_file(
            file_name=filename, crystal_symmetry=obj.crystal_symmetry()
        )

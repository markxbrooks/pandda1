        ...
        s_ = (
            ...
            rms_z_scores="\n\t\t".join(
                [
                    ...
                    for k, v in sorted(self.rms_z_scores.items())
                ]
            ),
        )
...
class EdstatsResults(object):
    ...
    def __str__(self):
        ...
        s_ = (
            ...
            global_scores=(str(self.global_scores).replace("\n", "\n\t\t")),
            n_results=(len(self.results_table)),
        )
    ...
    def get(self, residue_group=None, residue_group_label=None):
        ...
        # Use 'in' operator instead of .contains() for pandas MultiIndex compatibility
        if rg_label not in self.results_table.index:
            raise KeyError(
                "The given label does not have a corresponding result: {}".format(
                    rg_label
                )
            )
        ...
        for i, line in enumerate(output):
            ...
            if line.startswith("Overall mean Biso:"):
                ...
                for incr in [2, 3, 4]:
                    ...
                    score, main_val, side_val, all_val = (
                        output[i + incr].strip().split()
                    )
...
class CalculateEdstatsScores(object):
    ...
    def run_edstats_basic(
        ...
        if not os.path.exists(dataset.model.filename):
            ...
            raise IOError(
                "PDB file for edstats does not exist! {!s}".format(
        ...
        if not os.path.exists(dataset.data.filename):
            ...
            raise IOError(
                "MTZ file for edstats does not exist! {!s}".format(
    ...
    def _check_valid_f_label(self, f_label, dataset):
        ...
        if (f_label is not None) and (
            f_label not in dataset.data.crystal.column_labels
        ):
            raise Sorry(
    ...
    def _run_edstats_in_temp_dir_and_return_buffer(self, dataset, f_label):
        ...
        table_str_io = io.StringIO(str(table_str))

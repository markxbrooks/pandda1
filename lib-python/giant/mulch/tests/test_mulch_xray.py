...
def test_GetIsotropicMillerArrayScaler(five_baz2b_test_datasets_mcd):
    ...
    assert scale_data.info()["scaling_b_factor"] == pytest.approx(
        -0.0013033300638198853
    )
    ...
    assert (wilson_b_target_scaled - wilson_b_target_unscaled) == pytest.approx(
        -0.0013331343517322125, abs=1e-6
    )
...
def test_GetMultiDatasetMillerArrayScalers(five_baz2b_test_datasets_mcd):
    ...
    assert scaling_objects_dict["BAZ2BA-x434.dimple.pdb"].info()[
        "scaling_b_factor"
    ] == pytest.approx(0.0)
    assert scaling_objects_dict["BAZ2BA-x434.dimple.pdb"].info()[
        "scaled_ln_dev"
    ...
    assert scaling_objects_dict["BAZ2BA-x432.dimple.pdb"].info()[
        "scaling_b_factor"
    ] == pytest.approx(0.24365846067667007)
    assert scaling_objects_dict["BAZ2BA-x432.dimple.pdb"].info()[
        "scaled_ln_dev"
    ] == pytest.approx(93.9842415431974)
    assert scaling_objects_dict["BAZ2BA-x432.dimple.pdb"].info()[
        "unscaled_ln_dev"
    ] == pytest.approx(174.4268864939686)
    assert scaling_objects_dict["BAZ2BA-x432.dimple.pdb"].info()[
        "scaled_ln_rmsd"
    ] == pytest.approx(0.06364252415606744)
    assert scaling_objects_dict["BAZ2BA-x432.dimple.pdb"].info()[
        "unscaled_ln_rmsd"
    ] == pytest.approx(0.1015309431155802)
...
def test_MapGetterPrepper(five_baz2b_test_datasets_mcd):
    ...
    if True:
        ...
        assert ed_data[(1, 4, 10)] / np.std(ed_data) == pytest.approx(
            0.0315718853509
        )  # rmsd-scaled value
        assert ed_data[(1, 4, 10)] / ed_map.unit_cell().volume() == pytest.approx(
            0.00692379942548
        )  # volume-scaled value

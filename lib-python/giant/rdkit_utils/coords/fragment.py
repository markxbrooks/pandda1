...
def calculate_rmsd_between_fragged_files_using_symmetry(
    file1, file2, delete_structures=True
):
    """Take a pair of homologous fragmented molecule wrappers and return a list of the rmsds between the paired fragments in each molecule, minimising over equivalent models"""

    # Map file2 to file1 using symmetry
    tempfile = map_to_reference_using_symmetry(
        refpdb=file1, movpdb=file2, pdbout=file1.replace(".pdb", ".temp")
    )
...
def calculate_rmsd_between_fragged_files(file1, file2):
    """Take a pair of homologous fragmented molecule wrappers and return a list of the rmsds between the paired fragments in each molecule"""
...
def calculate_rmsd_between_fragged_mols(mol1, mol2):
    ...
    assert (
        len(list(set([at.GetMonomerInfo().GetChainId() for at in mol1.GetAtoms()])))
        == 1
    ), "MORE THAN ONE CHAIN PRESENT IN MOL!"
    assert (
        len(list(set([at.GetMonomerInfo().GetResidueName() for at in mol1.GetAtoms()])))
        == 1
    ), "MORE THAN ONE RESIDUE NAME PRESENT IN MOL!"
    assert (
        len(
            list(
                set([at.GetMonomerInfo().GetResidueNumber() for at in mol1.GetAtoms()])
            )
        )
        == 1
    ), "MORE THAN ONE RESIDUE NUMBER PRESENT IN MOL!"

    # Collect the fragments for mol1
    mol1_inscodes = sorted(
        list(set([at.GetMonomerInfo().GetInsertionCode() for at in mol1.GetAtoms()]))
    )
    ...
    for match_pat in match_patterns:
        ...
        for mol1_inscode in mol1_inscodes:
            ...
            mol1_frag_atms = [
                ...
                if at.GetMonomerInfo().GetInsertionCode() == mol1_inscode
            ]
            # Get the equivalent atoms in mol2
            mol2_frag_atms = [
                mol2.GetAtomWithIdx(match_dict[at.GetIdx()]) for at in mol1_frag_atms
            ]

            # Get the equivalent mol2 inscode for this mol1 inscode
            mol2_inscodes.append(
                list(
                    set(
                        [
                            at.GetMonomerInfo().GetInsertionCode()
                            for at in mol2_frag_atms
                        ]
                    )
                )[0]
            )
        ...
        match_pattern_fragment_rmsds.append(
            (mol1_inscodes, mol2_inscodes, fragment_rmsds, total_rmsd)
        )
    ...
    minimising_match_pattern = [
        r for r in match_pattern_fragment_rmsds if r[3] == min_rmsd
    ][0]

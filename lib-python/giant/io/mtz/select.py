def select_factors_and_phases_for_map(cols, maptype="2FOFC"):
    ...
    if maptype not in ["2FOFC", "FOFC"]:
        raise Exception(
            "MapType not in allowed map types: {!s} not in {!s}".format(
                maptype, MAP_TYPES
            )
        )
    ...
    if maptype == "2FOFC":
        if ("2FOFCWT" in cols) and ("PH2FOFCWT" in cols):
            ...
            raise ValueError(
                "Failed to select STRUCTURE FACTOR Columns for Map Type {!s}. \nColumns : {!s}".format(
                    maptype, cols
                )
            )
    elif maptype == "FOFC":
        ...
        raise NotImplementedError(
            "MapType does not have functionality programmed: {!s}".format(maptype)
        )
    # Return
    return lab_dict

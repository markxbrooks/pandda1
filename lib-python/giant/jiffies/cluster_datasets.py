...
DESCRIPTION = """
    A tool to cluster sets of pdb and/or mtz wrappers.
...
def regex_label(pattern, string):
    ...
    if len(matches) == 0:
        raise Sorry(
            "Regex did not match any part of string: \n\tPattern {}\n\tString {}".format(
                pattern, string
            )
        )
    if len(matches) > 1:
        raise Sorry(
            "Regex matched multiple parts of string: \n\tPattern {}\n\tString {}\n\tMatches {}".format(
...
def validate_params(params):
    # Validate input wrappers
    if not (params.input.pdb or params.input.mtz):
        raise Sorry(
            "No pdb/mtz wrappers have been provided: specify with input.pdb or input.mtz"
        )
...
def run(params):
    ...
    # Define output_file_function to copy or symlink wrappers as needed
    ...
    if params.input.labelling_regex:
        ...
        label_func = lambda s: regex_label(
            pattern=params.input.labelling_regex, string=s
        )
    ...
    logger.heading("Processing input pdb/mtz wrappers")
    ...
    if (n_pdb > 0) and (n_mtz > 0):
        raise Sorry("Must provide either PDB wrappers or MTZ wrappers but not both")
    ...
    if n_pdb > 0:
        crystal_summaries = [
            CrystalInfo.from_pdb(pdb_file=f, name=lab)
            for f, lab in zip(input_files, labels)
        ]
    else:
        crystal_summaries = [
            CrystalInfo.from_mtz(mtz_file=f, name=lab)
            for f, lab in zip(input_files, labels)
        ]
    ...
    for cg in crystal_groups:
        ...
        sg_name = "sg-{}".format(cg.space_groups[0].replace(" ", "_"))

        logger.subheading(
            "Space Group {}: {} dataset(s)".format(cg.space_groups[0], len(cg.crystals))
        )
        ...
        logger(
            "\nClustering {} unit cells using LCV cutoff of {}".format(
                len(cg.crystals), params.clustering.lcv_cutoff
            )
        )
        ...
        for i_cg2, cg2 in enumerate(sg_crystal_groups):
            ...
            logger("Copying wrappers to output directory")

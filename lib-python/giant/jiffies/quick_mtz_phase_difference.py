...
DESCRIPTION = """
Calculate the average phase difference between sets of mtz wrappers using cphasematch from CCP4.
"""
...
def run(params):
    ...
    if not os.path.exists(params.output.merged_mtz):
        ...
        raise Failure("giant.mtz.merge has failed to merge the mtz wrappers")
        ...
        for i_1 in range(1, len(params.input.mtz) + 1):
            ...
            match.extend_args(
                [
                    ...
                    params.output.phasematch_mtz,
                    "-colin-fo",
                    "{}-{},{}-{}".format(fo1, i_1, fo1, i_1),
                    "-colin-fc-1",
                    "{}-{},{}-{}".format(fc1, i_1, fc1, i_1),
                    "-colin-fc-2",
                    "{}-{},{}-{}".format(fc2, i_2, fc2, i_2),
                ]
            )
            match.run()
            march.write_output(
                params.output.phase_log_template + "-{}-{}.log".format(i_1.i_2)
            )

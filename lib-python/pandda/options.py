...
# Add project root to PYTHONPATH before importing giant or other project modules
# This ensures that giant, pandda, and other modules can be imported
parent_dir = Path(__file__).parent.parent.resolve()
parent_dir_str = str(parent_dir)
# Normalize path separators for cross-platform compatibility
parent_dir_normalized = parent_dir_str.replace("\\", "/")

# Check if project root is already in PYTHONPATH (try both formats)
pythonpath_dirs = [str(Path(p).resolve()) for p in sys.path]
parent_in_path = any(
    str(Path(p).resolve()) == parent_dir
    or p == parent_dir_str
    or p == parent_dir_normalized
    or p.replace("\\", "/") == parent_dir_normalized
    for p in sys.path
)

if not parent_in_path:
    sys.path.insert(0, parent_dir_str)
    # Also try adding normalized version if different
    if (
        parent_dir_normalized != parent_dir_str
        and parent_dir_normalized not in sys.path
    ):
        sys.path.insert(0, parent_dir_normalized)
else:
    # Path already in sys.path, but verify giant can be imported
    try:
        import giant
    except ImportError:
        # Force add the path even if it seems to be there
        sys.path.insert(0, parent_dir_str)

import giant.logs as lg  # noqa: E402
...
class Options(object):
    ...
    def multiprocessing_objects(self, config):
        ...
        self.remote_nodes = config.processing.remote_nodes
        self.cpus_per_remote_node = config.processing.cpus_per_remote_node
        self.h_vmem = config.processing.h_vmem
        self.m_mem_free = config.processing.m_mem_free

        # Parameters for local jobs
        self.local_cpus = config.processing.cpus
    ...
    def preprocess_objects(self, config):
        ...
        #####
        # Dataloader
        # Verify project root is in sys.path before importing
        parent_dir = Path(__file__).parent.parent.resolve()
        parent_dir_str = str(parent_dir)
        ...
        from giant.mulch.selectors import (  # RandomDatasetSelector,
            SortedDatasetSelector,
            SoulmateSelector,
        )
        from giant.mulch.sorters import HighResolutionSorter
        ...
        if config.input.filter.pdb is not None:
            from giant.mulch.dataset import ModelAndData
        ...
        #####
        # Input data checks
        from giant.mulch.checks import DiffractionArrayChecker, MultiDatasetChecker
        ...
        from giant.mulch.xray.get_data import (
            LoadFirstValidMillerArrayFromListOfColumnOptions,
        )
        ...
        copy_input_files = CopyPanddaInputFiles(
            output_dir=(config.output.out_dir / "processed_datasets"),
        )
        ...
        write_initial_dataset_summary = WritePanddaDatasetSummary(
            ...
            dataset_dir=(config.output.out_dir / "processed_datasets"),
        )
        ...
        self.dataset_initialiser.map_resolution_factor = (
    ...
    def process_objects(self, config):
        ...
        from giant.mulch.transform.maps import GetWarpedMapGrid, GetWarpedMapLoader
        ...
        from pandda.analyse.evaluate.maps import GetStandardSparseMapWriter
        ...
        get_pandda_model_task = GetPanddaModelProcessor(
                ...
                find_events=BasicPanddaFindEvents(
                    ...
                    find_clusters=BasicClusterFinder(
                        grid_clustering_cutoff=float(
                            config.params.z_map_analysis.agglomerative_hierarchical.clustering_cutoff
                        ),
                        negative_values=bool(
                            config.params.z_map_analysis.negative_values
                        ),
            ...
            get_model_outputter=GetPanddaStatisticalModelOutputter(
                output_dir=(config.output.out_dir / "analysis"),
                processor=self.process_in_shell,
            ),
        ...
        self.run_pandda_model = RunPanddaModel(
            ...
            output_dir=(config.output.out_dir / "shells"),
        ...
        # Results function
        #
        from pandda.analyse.output import MakePanddaResultsHtml, PanddaResultsOutputter

        #
        self.write_results = PanddaResultsOutputter(
            output_dir=(config.output.out_dir / "results"),
            write_html=MakePanddaResultsHtml(
                output_directory=(config.output.out_dir / "html"),
            ),
        )
        #
        ###
    ...
    def output_objects(self, config):
        ...
        self.dump_config_to_json = DumpConfigToJson(
            output_path=(config.output.out_dir / "params.json"),
        )

        from pandda.output import DumpDictToJson

        self.dump_results_to_json = DumpDictToJson(
            output_path=(config.output.out_dir / "results.json"),
        )
        ...
        self.make_html_output = MakeMainPanddaHtmlPage(
            output_directory=(config.output.out_dir / "html"),
            dataset_html=str(
                self.dataset_initialiser.write_output.write_html.output_path
            ),

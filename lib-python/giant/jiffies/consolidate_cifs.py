...
from giant.mulch.dataset import CrystallographicModel
from giant.phil import log_running_parameters
from giant.refinement.restraints.cif_tools import CifManager
from giant.refinement.restraints.consolidate import ConsolidateRestraintsAndUpdateModel
...
DESCRIPTION = """
    A script to make using multiple CIF wrappers and LINKs a little bit less painful.
"""
...
output {
    ...
    log = None
        ...
        .type = str
    phenix_params = consolidated_phenix_refine.params
        .help = 'input parameters for phenix.refine'
        .type = str
}
settings {
    overwrite = False
...
def set_get_log(params):
    ...
    if params.output.log is not None:
        ...
        params.output.log = str(pl.Path(params.output.cif).with_suffix(".log"))
    else:
...
def run(params):
    ...
    logger.heading("Reading input wrappers")

    cif_managers = [CifManager.from_file(p) for p in params.input.cif]
    ...
    model = (
        CrystallographicModel.from_file(params.input.pdb)
    ...
    if model is not None:
        ...
        logger.subheading("Input Atomic Model")
    ...
    logger.subheading("Consolidating and updating restraints")
    ...
    # Write output file
    logger.subheading("Writing output structures")

    result.write_pdb(params.output.pdb)
    result.write_cif(params.output.cif)
    result.write_phenix_params(params.output.phenix_params)

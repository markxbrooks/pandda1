...
class IsotropicBfactorScalingFactory(object):
    ...
    def calculate_scaling(
        ...
        from scitbx.math import approx_equal_relatively, scale_curves
        ...
        interpolator = scale_curves.curve_interpolator(
            d_star_sq_min, d_star_sq_max, self._npoints
        )
        ...
        while n_iter < max_iter:
            ...
            if approx_equal_relatively(0.0, scaling_b_factor, 1e-6):
                logger.debug(
                    "Scaling is approximately 0.0 ({}) - stopping".format(
                        scaling_b_factor
                    )
                )
                break
        ...
        lsc.info = collections.OrderedDict(
            ...
            unscaled_ln_rmsd=(
                flex.log(lsc.ref_values) - flex.log(lsc.scl_values)
            ).norm()
            / (lsc.ref_values.size() ** 0.5),
            scaled_ln_rmsd=(flex.log(lsc.ref_values) - flex.log(lsc.out_values)).norm()
            / (lsc.ref_values.size() ** 0.5),
            unscaled_ln_dev=flex.sum(
                flex.abs(flex.log(lsc.ref_values) - flex.log(lsc.scl_values))
            ),
            scaled_ln_dev=flex.sum(

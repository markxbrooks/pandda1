...
def generate_group_idxs(group_vals):
    ...
    for val, sel_idxs in itertools.groupby(
        list(range(num_vals)), key=lambda i: sorted_val[i]
    ):
...
def find_connected_groups(connection_matrix):
    ...
    assert not set(connection_matrix.flatten()).difference(
        set([0, 1])
    ), "CONNECTION MATRIX MUST CONSIST OF 1s AND 0s ONLY"
    ...
    connected_groups = scipy.cluster.hierarchy.fcluster(
        Z=l, t=0.01, criterion="distance"
    ).tolist()

...
def phenix_find_tls_groups(filename=None, hierarchy=None):
    ...
    assert [filename, hierarchy].count(
        None
    ) == 1, "Must supply either filename or hierarchy"
...
def make_tls_isotropic(t, l, s, l_and_s_in_degrees=True):
    ...
    from mmtbx.tls import analysis, decompose
    ...
    tls = analysis.tls_from_motions(
        ...
        v_x=matrix.rec(
            (1, 0, 0), (3, 1)
        ),  # don't need to provide actual axes as no longer defined

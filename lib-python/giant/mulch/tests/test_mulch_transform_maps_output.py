...
def test_MaskedNativeMapMaker(five_baz2b_test_datasets_mcd_labelled, tmp_path):
    ...
    map_data = np.ones(map_grid.masks["inner"].mask_size)
    ...
    output_map_data = make_native_map(
        map_data=map_data,
        dataset=five_baz2b_test_datasets_mcd_labelled.datasets[
            "BAZ2BA-x434.dimple.pdb"
        ],
    ...
    assert np.array(output_map_data[large_select]).mean() == pytest.approx(
        0.7729, abs=1e-4
    )
    assert np.array(output_map_data[large_select]).std() == pytest.approx(
        0.1597, abs=1e-4
    )
...
def test_NativeMapMaker(five_baz2b_test_datasets_mcd_labelled, tmp_path):
    ...
    map_data = np.ones(map_grid.grid_size_1d()).reshape(map_grid.grid_size())

    from giant.mulch.transform.maps.output import NativeMapMaker

    make_native_map = NativeMapMaker(
        map_grid=map_grid,
        sites_mask=[(10.0, 35.0, 25.0), (10.0, 35.0, 30.0)],

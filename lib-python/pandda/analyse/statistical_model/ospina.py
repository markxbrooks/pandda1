...
def estimate_true_underlying_sd(
    obs_vals, obs_error, est_mu=None, est_sigma=1e-16, try_number=1
):
    ...
    answer = abs(
        fsolve(
            func=variable_sigma_d_log_likelihood,
            x0=est_sigma,
            args=(est_mu, obs_vals, obs_error),
        )
    )[0]

    if answer > 2e40:
        est_sigma = est_sigma / 1000.0
        answer = estimate_true_underlying_sd(
            obs_vals=obs_vals,
            obs_error=obs_error,
            est_mu=est_mu,
            est_sigma=est_sigma,
...
if __name__ == "__main__":
    ...
    for attempt in range(num_cycles):
        ...
        out = estimate_true_underlying_sd(
            obs_vals=obs_vals, obs_error=obs_error, est_sigma=0.1
        )
    ...
    print(
        "TIME TAKEN PER CYCLE: {!s} (Millionths)".format(
            int(1000000.0 * (end_t - start_t) / num_cycles)
        )
    )

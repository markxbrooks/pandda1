...
class GridPartition(object):
    ...
    def __str__(self):
        ...
        s_ = (
            ...
            masked_points=(
                (np.prod(self.grid_size) - self.grid_mask.mask_size)
    ...
    def validate(self):
        ...
        if self.grid_mask is not None:
            ...
            assert len(self.partition_mappings) == np.prod(self.grid_size)
...
class AssignSites(object):
    ...
    def __call__(self, query_sites):
        ...
        nn_dists, nn_groups = tree.query(query_sites.astype(float))

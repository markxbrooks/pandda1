...
def break_and_rename_mol_to_file(infile, outfile):
    ...
    assert (
        len(orig_mol.getChains()) == 1
    ), "MORE THAN ONE CHAIN PRESENT IN FILE! {!s}".format(infile)
    assert (
        len(orig_mol.getResidues()) == 1
    ), "MORE THAN ONE RESIDUE PRESENT IN FILE! {!s}".format(infile)

    # Keep the atom numbers (these are conserved in the mappings)
    atom_numbers = dict(
        [(a.atomname, a.serialnum) for a in orig_mol.getResidues()[0].atoms]
    )
    ...
    for i, frag in enumerate(frags):
        ...
        assert (
            len(raw_frag.getChains()) == 1
        ), "FRAGMENT HAS MORE THAN ONE CHAIN? {!s}\n{!s}".format(infile, raw_block)
        assert (
            len(raw_frag.getResidues()) == 1
        ), "FRAGMENT HAS MORE THAN ONE RESIDUE? {!s}\n{!s}".format(infile, raw_block)

...
class ZScoreClassifier(object):
    ...
    def __init__(
        ...
        self.column_suffix = (
            str(column_suffix) if (column_suffix is not None) else column_suffix
        )
    ...
    def __call__(self, dataframe):
        ...
        for c in self.columns:
            ...
            exclude = zscores > self.max_z_score
            # Create union with existing set
            outliers = outliers.union(list(series.index[exclude].values))

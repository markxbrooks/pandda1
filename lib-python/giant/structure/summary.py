...
class StructureSummary(object):
    ...
    def __init__(
        self, pdb_input=None, pdb_hierarchy=None, pdb_file=None, mtz_file=None
    ):
        """Summarise some characteristics of a structure"""

        assert [pdb_file, pdb_input, pdb_hierarchy].count(
            None
        ) == 2, "Provide pdb_file or pdb_input OR pdb_hierarchy"
        if pdb_file:
    ...
    def normalise_b_factors(self, root=None):
        if root is None:
            root = self.pdb_hierarchy
        return normalise_b_factors_to_z_scores(
            pdb_hierarchy=root, b_factor_summary=self.b_factors
        )
    ...
    def get_atom_group_summary(self, atom_group):
        return atomGroupSummary(
            atom_group=atom_group, global_b_factor_statistics=self.b_factors
        )
    ...
    def to_html(self, fname):
        write_html_summary(
            fname=fname,
            atom_group_summaries=[
                self.get_atom_group_summary(ag)
                for ag in self.pdb_hierarchy.atom_groups()
            ],
        )

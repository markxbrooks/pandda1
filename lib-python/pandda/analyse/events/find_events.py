...
class BasicPanddaFindEvents(object):
    ...
    def __call__(
        ...
        try:
            ...
            logger.warning(f"Error finding events for {dataset_key}: skipping")
            return []
        ...
        event = collections.OrderedDict(
            [
                ...
                (
                    ...
                    tuple(
                        round(v, 3)
                        for v in dataset.model.alignment.ref2nat(
                ...
                (
                    "xyz_peak_ref",
                    tuple(round(v, 3) for v in map_grid.grid2cart([cluster.peak()])[0]),
                ),
                (
                    "xyz_centroid_ref",
                    tuple(
                        round(v, 3) for v in map_grid.grid2cart([cluster.centroid()])[0]
                    ),
                ),
            ]
        )

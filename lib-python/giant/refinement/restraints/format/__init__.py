...
from .phenix import PhenixRestraintCollectionFormatter
from .refmac import RefmacRestraintCollectionFormatter
...
class WriteRestraints(object):
    ...
    def __init__(
        ...
        self.output_path_dict = (
            output_path_dict if output_path_dict is not None else dict()
        )

        self.formatters = {k: get_restraint_formatter(k) for k in formats}

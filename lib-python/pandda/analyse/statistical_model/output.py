...
from .graphs import GetMakePanddaStatisticalModelGraphs
...
class PanddaStatisticalModelOutputter(object):
    ...
    def __call__(self, statistical_model, **kwargs):

        output_files = {}

        for func in self.functions:

            of = func(statistical_model=statistical_model, **kwargs)
...
class GetPanddaStatisticalModelOutputter(object):
    ...
    def __call__(self, label):
        ...
        return PanddaStatisticalModelOutputter(
            functions=[g(label=label) for g in self.getters],
        )

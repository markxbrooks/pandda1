...
from giant.common.clustering import find_connected_groups, generate_group_idxs
...
def pairwise_distances_between_sites(sites1, sites2):
    ...
    dm = sites1.reshape((n1, 1, 3)).repeat(n2, axis=1) - sites2

    return np.sqrt((dm * dm).sum(axis=-1))
...
class ClusterFilterList(object):
    ...
    def __call__(self, clusters, **kw_args):
        ...
        for f in self.filters:
            ...
            clusters = f(clusters=clusters, **kw_args)
...
class PeakAndSizeClusterFilter(object):
    ...
    def __call__(self, clusters, map_grid, **kw_args):

        clusters = self.filter_size_and_peak(
            clusters=clusters,
            min_peak=(self.min_blob_z_peak),
            min_size=(self.min_blob_volume / (map_grid.grid_spacing() ** 3)),
        )

        return clusters

    def filter_size_and_peak(
        self,
        clusters,
        min_peak,
        min_size,
...
class GroupNearbyClustersFilter(object):
    ...
    def __call__(self, clusters, map_grid, **kw_args):
        ...
        clusters = self.group_clusters(
            clusters=clusters,
            grid_distance_cutoff=(self.distance_cutoff / map_grid.grid_spacing()),
        )
...
class ContactsClusterFilter(object):
    ...
    def find_clusters_with_contacts(
        self,
        clusters,
        map_grid,
        dataset,
        ...
        ref_sites_cart = dataset.model.alignment.nat2ref(self.get_filter_sites(dataset))

        ref_sites_grid = map_grid.cart2grid(ref_sites_cart)
...
class SymmetryClusterFilter(object):
    ...
    def remove_symmetry_related_clusters(
        self,
        clusters,
        map_grid,
        dataset,
        ...
        cluster_points_cart = [
            dataset.model.alignment.ref2nat(map_grid.grid2cart(c.points))
            for c in clusters
        ]

        cluster_points_frac = [
            d_unit_cell.fractionalize(points) for points in cluster_points_cart
        ]
        ...
        for c_lab, c_idxs in generate_group_idxs(sym_equiv_groups):
            ...
            if len(c_idxs) == 1:
                out_clusters.append(clusters[c_idxs[0]])
            ...
            if max_contacts == 0:
                ...
                out_clusters.append(clusters[i_best])

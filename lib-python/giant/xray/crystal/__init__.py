...
class CrystalInfo(object):
    ...
    def summary(self):
        ...
        lines.append(
            "Space group: {}".format(self.space_group.type().lookup_symbol())
        ...
        lines.append(
            "R-free: {}".format(
                round(self.r_free, 3) if (self.r_free is not None) else str(self.r_free)
            )
        )

        lines.append(
            "R-work: {}".format(
                round(self.r_work, 3) if (self.r_work is not None) else str(self.r_work)
            )
        )

...
from giant.html.summary import HtmlSummary, ImageEmbedder
...
class MakePanddaResultsHtml(object):
    ...
    def get_contents(
        ...
        main_block.append(
            self.make_events_tab(
                ...
                test_dataset_keys=sorted(
                    [k for k, v in dataset_dicts["test"].items() if (v is True)]
                ),
        ...
        main_block.append(
            self.make_datasets_tab(
                dataset_dicts=dataset_dicts,
                all_dataset_keys=sorted(dataset_dicts["high_resolution"].keys()),
                output_graphs=output_files.get("graphs", {}),
                output_tables=output_files.get("tables", {}),
            )
        )
            ...
            contents=[
                ...
                divs.ScrollX(
                    contents=[
                        ...
                        for site_num, site_image in sorted(
                            output_graphs.get("site_events", {}).items()
                        )
                    ],
                ),
                divs.Block(
        ...
        block = divs.Block(
            ...
            contents=HtmlSummary.format_summary(
                (
                    ...
                    test_datasets=" ".join([d_label.format(k=k) for k in info["test"]]),
                    train_datasets=" ".join(
        ...
        block = divs.Block(
            contents=[
                ...
                self.make_pretty_block(
                    ...
                    text=("Uncertainties of training and test datasets."),
                ),
            ],
        )
    ...
    def get_datasets_header(
        self,
        dataset_dicts,
    ):

        train_keys = [k for k, v in dataset_dicts["train"].items() if (v is True)]
        test_keys = [k for k, v in dataset_dicts["test"].items() if (v is True)]
    ...
    def get_datasets_main(
        ...
        for dkey in all_dataset_keys:
            ...
            d_info = {k: d.get(dkey) for k, d in dataset_dicts.items()}
            ...
            contents=[
                ...
                divs.ScrollX(
                    ...
                    contents=[
                        ...
                        for event_num, event_image in sorted(
                            output_graphs.get("bdc_estimation", {})
                            .get(dataset_key)
                            .items()
                        )
                    ],
    ...
    def make_pretty_block(
        self, title, image, text, width=12, text_width=6, image_width=6
    ):

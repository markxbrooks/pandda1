...
class GetInterestingResnames(object):
    ...
    def __init__(
        ...
        self.ignore_common_molecules = bool(ignore_common_molecules)

        self.include_resnames_list = (
            list(include_resnames_list) if (include_resnames_list is not None) else None
        )

        self.ignore_resnames_list = (
            list(ignore_resnames_list) if (ignore_resnames_list is not None) else None
        )
        ...
        if self.ignore_common_molecules is False:
            ...
            int_resnames.update(
                [
                    r
                    for r in unq_resnames
                    if common_residue_names_get_class(r)
                    not in [
                        "common_amino_acid",
                        "common_element",
                        "common_water",

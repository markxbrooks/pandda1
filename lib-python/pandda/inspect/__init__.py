...
class Notices(object):
    def __init__(self):
        ...
        box = gtk.HBox(spacing=5)
        label = gtk.Label(
            "NOTE: This only applies to the PanDDA maps (those loaded from wrappers ending with *.ccp4)"
        )
        label.props.width_chars = 100
        label.set_justify(gtk.JUSTIFY_CENTER)
        label.set_line_wrap(True)
...
class SplashScreen(object):
    ...
    def show_menu(self):
        ...
        # ---------------------------
        button = gtk.Button(
            label="Read this important information regarding map contouring"
        )
        button.child.set_padding(5, 5)
...
class Event(object):
    ...
    def __init__(self, info):
        ...
        self.event_num = int(info.name[1]) if not np.isnan(info.name[1]) else None
        ...
        self.site_num = (
            int(info["site_num"]) if not np.isnan(info["site_num"]) else None
        )
        ...
        self.z_peak = round(info["z_peak"], 1) if not np.isnan(info["z_peak"]) else None

        self.z_mean = info["z_mean"] if not np.isnan(info["z_mean"]) else None

        self.cluster_size = (
            int(info["cluster_size"]) if not np.isnan(info["cluster_size"]) else None
        )

        self.xyz = tuple(info[c] for c in ["x", "y", "z"] if not np.isnan(info[c]))

        if len(self.xyz) != 3:
            self.xyz = None

        self.added_ligand_names = [s for s in info["Ligand Names"].split(",") if s]
...
class GetEventFiles(object):
    ...
    def __call__(
        ...
        dataset_dir = self.pandda_directory / "processed_datasets" / event.dtag

        assert dataset_dir.exists(), "Directory does not exist: {}".format(event.dtag)

        # Identify dataset subdirectories and wrappers
        ...
        output_model_link = model_dir / "{dkey}-pandda-model.pdb".format(
            dkey=event.dtag
        )
        ...
        input_model = self.pandda_directory / pl.Path(
            dataset_files["structure"]
        ).relative_to(self.pandda_path_prefix)

        input_data = self.pandda_directory / pl.Path(dataset_files["data"]).relative_to(
            self.pandda_path_prefix
        )

        output_data = self.pandda_directory / pl.Path(
            dataset_files["output_data"]
        ).relative_to(self.pandda_path_prefix)

        # events are optional
        if ("event_data" in dataset_files) and (event.event_num is not None):
            event_data = self.pandda_directory / pl.Path(
...
class GetNextModelFile(object):
    ...
    def last_file(self):
        ...
        fitted_outputs = list(
            map(str, sorted(self.model_directory.glob(self.output_prefix + "*")))
        )
...
class LoadEventModelsAndMaps(object):
        ...
        molecules.update(
            self.load_and_assign_mols_for_models(
                pdb_path=(
                    ...
                    if os.path.exists(self.event_files["output_model_link"])
    ...
    def load_and_assign_mols_for_models(
        self,
        pdb_path,
    ):
    ...
    def load_and_assign_mols_for_maps(
        self,
        mtz_path,
    ):
        """

        Ordering of columns in MTZ wrappers:
...
class EventModelMapHandler(object):
    ...
    def load_all(self):
        ...
        self.molecules.update(self.load_models_and_maps())

        if self.event_files["event_data"] is None:
            self.make_custom_event_map(0.0)

        set_main_coot_molecule(self.molecules["model"])
    ...
    def load_original_model_not_active(self):
        ...
        imol = read_pdb(self.event_files["input_model"])
...
class LigandHandler(object):
    ...
    def update(self):
        ...
        cifs = sorted(self.ligand_directory.glob("*.cif"))

        pdbs = [p.with_suffix(".pdb") for p in cifs]
        ...
        self.ligand_cif_dict = {self.get_cif_name(p): str(p) for p in cifs}
    ...
    def get_i_ligand_name(self, i_ligand=None):
        ...
        return self.get_cif_name(self.ligand_cifs[i_ligand])
...
class EventHandler(object):
    ...
    def __init__(
        ...
        self.ligand_handler = LigandHandler(
            ...
            show_ligands=(not os.path.exists(self.event_files["output_model_link"])),
    ...
    def load_all(self):
        ...
        # Load cif wrappers for any ligand that were previously added
    ...
    def merge_ligand_with_model(self):
        ...
        imol_model = self.model_map_handler.molecules.get("model")
...
class PanddaEventListController(object):
    ...
    def refresh(self):
        self.load_event(self.event_tracker.get())

    def go_to(self, i_event):
        self.load_event(self.event_tracker.set(i_event))

    def next_event(self):
        self.load_event(self.event_tracker.next())

    def prev_event(self):
        self.load_event(self.event_tracker.prev())
    ...
    def get_i_next_from_selection(self, selection):
        ...
        if len(indices) == 0:
            ...
            indices = sorted(((indices - i_current) % n_total) + i_current)

            # We know that there are at least two events so this should be safe!
            i_next = indices[0] if indices[0] != i_current else indices[1]
...
class UpdateOutput(object):
    ...
    def make_output_graphs(self):
        ...
        for e in event_dicts:
            e["colour"] = (
                "limegreen" if e["Ligand Placed"] else "red" if e["Viewed"] else "blue"
            )
...
class MakeNewLigand(object):
    ...
    def __init__(
        ...
        self.tmp_directory = self.output_directory / "make_ligand_tmp"
    ...
    def __call__(
        ...
        real_out_pdb = self.output_directory / (ligand_name + ".pdb")

        real_out_cif = real_out_pdb.with_suffix(".cif")

        if real_out_pdb.exists() or real_out_cif.exists():
            raise ValueError(
                "Output ligand wrappers already exist -- please choose a different id/name"
            )
        ...
        try:
            ...
            tmp_pdb, tmp_cif = generate_restraints(
                ...
                name=ligand_id,
                prefix=str(self.tmp_directory / ligand_name),
            )
    ...
    def validate(
        ...
        if len(ligand_id) == 0:
            ...
            raise ValueError("No ligand id provided")

        if len(ligand_id) != 3:

            raise ValueError("Ligand ID must be three characters")

        if len(ligand_name) == 0:

            raise ValueError("No ligand name provided")

        if len(ligand_smiles) == 0:

            raise ValueError("No ligand smiles provided")
    ...
    def handle(self, dialog, objects):
        ...
        while success is False:
            ...
            assert response is int(
                gtk.RESPONSE_ACCEPT
            ), "invalid response received ({} should be {})".format(
                response, int(gtk.RESPONSE_ACCEPT)
            )
            ...
            try:
                ...
                self.set_status(
                    objects,
                    "Error: change parameters and then click OK to re-run",
                )
                modal_msg(str(e))
                continue
            ...
            self.set_status(
                objects,
                "No wrappers generated but no error was raised - please contact developer",
            )
...
class GetMakeCustomEventMapWindow(object):
    ...
    def populate(self, window, starting_value):
        ...
        bdc_entry = gtk.Entry()
        bdc_entry.set_text(str(starting_value))
    ...
    def connect(self, objects):
        ...
        objects["entry"].connect(
            "activate",
            lambda x: [
                objects["adjustment"].set_value(float(objects["entry"].get_text())),
            ],
        )
        ...
        objects["adjustment"].connect(
            ...
            lambda x: [
                ...
                self.event_handler.model_map_handler.make_custom_event_map(
                    bdc_value=(1.0 - float(objects["adjustment"].get_value())),
                ),
            ],
        )
...
def validate_path(path):
    ...
    if not os.path.exists(str(path)):
        raise MissingFile("File does not exist: {}".format(str(path)))
...
class ProgressBar(Block):
    ...
    def __init__(
        self, data, width=12, add_counts=True, add_percentages=True, **kw_args
    ):
    ...
    def set_data(self, data, add_counts, add_percentages):
        ...
        total_width = float(np.sum([d["value"] for d in data]))
...
class DivFormatter(object):
    ...
    def __init__(self, div_class=Block, *args, **kwargs):
...
class CitationFormatter(DivFormatter):
    ...
    def __call__(
        ...
        main_div = div_class(text="<br>".join(text_lines), *args, **kwargs)
    ...
    def format_link(self, link, link_text):
        ...
        link_text = '<a href="{link}"><small>{link_text}</small></a>'.format(
            link=link,
            link_text=link_text,
        )
...
class MakePanddaInspectHtml(object):
    ...
    def __init__(
        ...
        self.output_path = pl.Path(self.get_path(["html", "inspect_html"]))
    ...
    def build_input_files_dict(self):
        ...
        json_file = self.output_directory / "results.json"
    ...
    def get_path(self, input_keys):
        ...
        output_path = self.output_directory / pl.Path(d).relative_to(
            self.input_path_prefix
        )

        return str(output_path)

    def unpack_inspector(
        self,
        inspector,
    ):
    ...
    def get_contents(
        ...
        contents.extend(self.get_header())
    ...
    def get_inspection_progress_bars(
        self,
        summary_dict,
    ):
        ...
        block1 = Block(
            ...
            contents=[
                ...
                Alert(
                    text="Total events: {}".format(summary_dict["n_blobs"]),
                    colour="info",
                    width=12,
                ),
                Alert(
                    text="Fitted ligands: {}".format(summary_dict["n_fitted"]),
                    colour="success",
                    width=4,
                ),
                Alert(
                    text="Unviewed: {}".format(summary_dict["n_unviewed"]),
                    colour="info",
                    width=4,
                ),
                Alert(
                    text="No ligand fitted: {}".format(summary_dict["n_empty"]),
                    colour="danger",
                    width=4,
                ),
            ],
        ...
        block2 = Block(
            ...
            contents=[
                ...
                Alert(
                    text="Sites with ligands: {}".format(summary_dict["n_sites_w_hit"]),
                    colour="info",
                    width=6,
                ),
            ],
        ...
        block3 = Block(
            ...
            contents=(
                [
                    ...
                    Alert(
                        text="Total ligands: {}".format(summary_dict["n_fitted"]),
                        colour="info",
                        width=12,
                    ),
                    Alert(
                ...
                if (summary_dict["n_fitted"] > 0)
                else [Alert(text="Nothing to show here yet: no fitted ligands")]
            ),
        )

        return [block1, block2, block3]

    def get_inspection_site_summary(
        self,
        summary_dict,
        output_files,
    ):
        ...
        blocks = [
            ...
            ScrollX(
                contents=[
                    ...
                    for site_num, n_events in sorted(site_counts.items())
                ],
            ),
        ]
    ...
    def get_inspection_event_summary(
        self,
        event_table,
    ):
    ...
    def build_event_table_summary_dict(
        self,
        event_table,
    ):
        ...
        n_blobs = len(event_table.index)

        n_sites = len(set(event_table["site_num"]))
        ...
        n_fitted = sum(event_table["Ligand Placed"])

        n_viewed = sum(event_table["Viewed"])

        n_empty = n_viewed - n_fitted

        n_unviewed = n_blobs - n_viewed
        ...
        n_hits_per_site = {
            i_site: sum(event_table["Ligand Placed"][event_table["site_num"] == i_site])
    ...
    def make_output(
        ...
        logger.info("Writing output HTML: {}".format(self.output_filename))
        ...
        logger.info("Output HTML written to {}".format(str(self.output_path)))
...
class PanddaInspect(object):
    ...
    def refresh_gui(self):
        ...
        self.labels["site_val"].set_label(str(self.controller.site_tracker.get() + 1))

        self.labels["site_tot"].set_label(
            str(self.controller.site_tracker.n_total)
        )  # move elsewhere only needs doing once...

        self.labels["rank_val"].set_label(str(self.controller.event_tracker.get() + 1))
        ...
        self.labels["dtag"].set_label("<b>" + str(e.dtag) + "</b>")

        self.labels["e_num"].set_label(
            "{event_num}/{n_total}".format(
                event_num=str(e.event_num),
                n_total=str(self.controller.event_counts.get(e.dtag, "?")),
            )
        )

        self.labels["e_1_bdc"].set_label(
            str(e.est_1_bdc) if e.est_1_bdc is not None else "n/a"
        )

        self.labels["zpeak"].set_label(
            str(round(e.z_peak, 3)) if e.z_peak is not None else "n/a"
        )

        self.labels["map_res"].set_label(str(e.map_resolution))

        self.labels["map_unc"].set_label(str(e.map_uncertainty))

        self.labels["rwork_rfree"].set_label("{} / {}".format(*e.rwork_rfree))
        ...
        self.buttons["merge-ligand"].child.set_text("Merge Ligand\nWith Model")
...
class ExpandEventTable(object):
    ...
    def __init__(
        ...
        self.keep_original_events = keep_original_events

        self.remove_datasets_with_events = remove_datasets_with_events
    ...
    def __call__(
        ...
        dummy_event_dicts = self.make_event_dicts_from_dataset_table(
            dataset_dicts=list(dataset_table.to_dict("records")),
        )
        ...
        if self.keep_original_events is False:
            ...
            event_dicts = list(event_table.to_records())

            if self.remove_datasets_with_events is True:

                event_dtags = sorted(set([e["dtag"] for e in event_dicts]))

                dummy_event_dicts = [
                    d for d in dummy_event_dicts if d["dtag"] not in event_dtags
                ]

        #

        df = self.make_pandda_event_table(
            event_dicts=(event_dicts + dummy_event_dicts),
        )
...
class GetPanddaInspectInputOutputFiles(object):
    ...
    def __init__(
        ...
        self.input_directory = pl.Path(input_directory)
    ...
    def __call__(self):
        ...
        ret_dict.update(self.get_input_files(output_files))

        ret_dict.update(self.get_output_files())

        return ret_dict

    def get_json_data(self):

        results_json = self.input_directory / "results.json"

        results_dict = json.loads(open(str(results_json), "r").read())

        return results_dict

    def get_input_files(self, output_files):

        remove_prefix = self.find_output_prefix(output_files["tables"]["dataset_info"])

        if self.mode == "events":

            input_events_csv = self.input_directory / (
                pl.Path(output_files["tables"]["events_table"]).relative_to(
                    remove_prefix
                )
            )

            input_sites_csv = self.input_directory / (
                pl.Path(output_files["tables"]["sites_table"]).relative_to(
                    remove_prefix
                )
            )
            ...
            input_events_csv = self.output_directory / (pl.Path("dummy_events.csv"))

            input_sites_csv = self.output_directory / (pl.Path("dummy_sites.csv"))
            ...
            make_event_table = ExpandEventTable(
                dataset_info_csv=str(
                    ...
                    / (
                        pl.Path(output_files["tables"]["dataset_table"]).relative_to(
                            remove_prefix
                        )
                    )
                ),
                dataset_tags=[
                    d
                    for (d, d_files) in output_files["dataset_files"].items()
    ...
    def get_output_files(self):
        ...
        if self.mode == "events":
            ...
            output_events_csv = str(out_dir / "pandda_inspect_events.csv")

            output_sites_csv = str(out_dir / "pandda_inspect_sites.csv")

        elif self.mode == "datasets":

            output_events_csv = str(out_dir / "pandda_inspect_events.datasets.csv")

            output_sites_csv = str(out_dir / "pandda_inspect_sites.datasets.csv")
...
if __name__ == "__main__":
    ...
    try:
        ...
        get_io_files = GetPanddaInspectInputOutputFiles(
            ...
            output_directory=(working_directory / "inspect"),
            mode=args.mode,
        )
        ...
        modal_msg(
            "Missing wrappers! Are you in the right directory?\n\nNo {}\n\npandda.inspect will now close".format(
                str(e)
            )
        )

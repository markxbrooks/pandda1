...
from .base_processors import BaseProcessor
from .luigi_sge import SGEJobTask
...
class LuigiFunctionWrapper(object):
    ...
    def __init__(
        ...
        self.output_filename = str(random_string(length=30)) + str(
            "" if output_file_suffix is None else output_file_suffix
        )

        self.output_path = pl.Path(shared_tmp_dir) / output_filename
...
class ProcessorLuigiSGE(BaseProcessor):
    ...
    def __call__(
        ...
        wrappers = [LuigiWrapper(f) for f in funcs]
        ...
        if result_loaders:
            results = [r() for r in result_loaders]
        else:
            results = [None for w in wrappers]

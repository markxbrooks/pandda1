...
from .graphs import MakeBdcEstimationGraph
...
class EventAnalyser(object):
    ...
    def __call__(
        ...
        all_event_fractions, all_event_correlations, all_global_correlations = (
            self.calculate_bdc_series_correlations(
        ...
        event_info = collections.OrderedDict(
            [
                (
                    "event_fraction",
                    round(event_fraction, 3),
                ),
                (
                    "bdc",
                    round(1.0 - event_fraction, 3),
                ),
                (
                    "global_correlation",
                    round(global_correlation, 3),
                ),
                (
                    "local_correlation",
                    round(local_correlation, 3),
                ),
                (
                    "output_files",
                    output_files,
    ...
    def calculate_bdc_series_correlations(
        self,
        reference_map_data,
        dataset_map_data,
        event_selection,
        ...
        event_fractions, event_correlations, global_correlations = (
            bc.calculate_feature_fraction_correlations(

...
class StatisticalMapDistributionsPlotter(PanddaPlotter):
    ...
    def plot(self, statistical_model, *args, **kwargs):

        mean_map_values = statistical_model.mu_map_data
        sadj_map_values = statistical_model.sigma_adjusted_map_data
...
class StatisticalMapUncertaintyDistributionPlotter(PanddaPlotter):
    ...
    def make_histogram(self, axis, values, n_bins=30, **kwargs):
    ...
    def __init__(
        ...
        self.plotters = [
            StatisticalMapDistributionsPlotter(
                output_path=str(output_dir / "statistical_map_distributions.png"),
            ),
            StatisticalMapUncertaintyDistributionPlotter(
                output_path=str(output_dir / "dataset_uncertainty_distribution.png"),
            ),
        ]

        self.processor = processor

    def __call__(self, statistical_model, *args, **kwargs):

        output_files = {}

        for plotter in self.plotters:

            of = plotter(statistical_model=statistical_model, *args, **kwargs)

...
from .evaluate import GetPanddaDatasetEvaluator, GetPanddaDatasetEvaluatorOutputter
...
class PanddaModelProcessor(object):
    ...
    def __init__(
        ...
        self.output_json_path = (
            pl.Path(output_json_path) if (output_json_path is not None) else None
        )
    ...
    def __call__(
        ...
        train_keys = list(train_datasets.keys())
        test_keys = list(test_datasets.keys())
        all_keys = list(set(train_keys).union(test_keys))
        train_only_keys = list(set(train_keys).difference(test_keys))
        train_and_test_keys = list(set(train_keys).intersection(test_keys))
    ...
    def extract_dataset_info(
        ...
        dataset_info = {
            ...
            "analysed_resolution": {
                dkey: (round(map_resolution, 3)) for dkey in datasets.keys()
            },
        }

        return dataset_info

    def extract_output_files_from_events(
        self,
        events_dict,
    ):
    ...
    def update_event_info(
        ...
        for dkey, d_events in events_dict.items():
            ...
            for e in d_events:
                e["map_resolution"] = round(map_resolution, 3)
    ...
    def load_maps_or_get_from_existing(
        self,
        datasets,
        existing_maps,
        map_resolution,
    ...
    def return_results(self, results_dict):
        ...
        if self.output_json_path is not None:
            ...
            logger("Output written to {}".format(str(self.output_json_path)))
...
class RunPanddaModel(object):
    ...
    def __call__(
        ...
        for (shell_res_high, shell_res_low), shell_datasets in dataset_shells.items():
            run_label = "shell-{high}-{low}".format(
                high="{:.2f}".format(shell_res_high),
                low="{:.2f}".format(shell_res_low),
            )

            output_json_path = self.output_dir / "{}-results.json".format(run_label)
            ...
            shell_records.append(
                collections.OrderedDict(
                    [
                        ...
                        (
                            "resolution_high",
                            round(shell_res_high, 3),
                        ),
                        (
                            "resolution_low",
                            round(shell_res_low, 3),
                        ),
                        (
                            "map_resolution",
                            round(shell_res_low, 3),
                        ...
                        (
                            "train",
                            sorted(shell_datasets["train"].keys()),
                        ),
                        (
                            "test",
                            sorted(shell_datasets["test"].keys()),
                        ),
                    ]
                )
            )
    ...
    def unpack_results(
        ...
        for i, s_records in enumerate(shell_records):
            ...
            s_json = pl.Path(s_records["json_path"])
        ...
        return_dict = {
            "events": sorted(all_events, key=lambda e: (e["dtag"], e["event_num"])),
            "shell_records": (shell_records),
            "dataset_records": make_sorted_dict(all_dataset_records),

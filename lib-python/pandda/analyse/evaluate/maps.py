    ...
    def __iter__(self):
        ...
        if self.output_dataset_map is True:
            ...
            filename = self.output_files.setdefault(
                "dataset_map",
                str(self.output_dir / "pandda_input_map.ccp4"),
            )
        ...
        if self.output_ground_map is True:
            ...
            filename = self.output_files.setdefault(
                "ground_map",
                str(self.output_dir / "pandda_ground_state_map.ccp4"),
            )
        ...
        if self.output_dataset_z_maps is True:
            ...
            filename = self.output_files.setdefault(
                "z_map",
                str(self.output_dir / "pandda_z_map.ccp4"),
            )
        ...
        if self.output_dataset_event_maps is True:
            ...
            for i, bdc in enumerate(self.bdc_values):
                ...
                filename = event_maps.setdefault(
                    i,
                    str(self.output_dir / "pandda_event_map_{:03d}.ccp4".format(i + 1)),
                )
...
class MakePanddaEvaluationMaps(object):
    ...
    def __call__(
        ...
        # Extract output wrappers for each dataset
        output_files = {}

...
def write_html_summary(fname, atom_group_summaries):
    ...
    for ag_sum in atom_group_summaries:
        ...
        columns.append(
            {
                "colour": "default",
                "message": round(float(flex.sum_sq(bfs) / bfs.size()) ** 0.5, 1),
            }
        )
        ...
        if (flex.sum_sq(bfz) / bfz.size()) ** 0.5 < 2.5:
            columns.append(
                {
                    "colour": "default",
                    "message": round(float(flex.sum_sq(bfz) / bfz.size()) ** 0.5, 1),
                }
            )
        else:
            columns.append(
                {
                    "colour": "danger",
                    "icon": "remove",
                    "message": round((flex.sum_sq(bfz) / bfz.size()) ** 0.5, 1),
                }
            )

        row_message = "" if (flex.sum_sq(bfz) / bfz.size()) ** 0.5 < 2.5 else "Check"
        row_colour = "success" if row_message == "" else "danger"

        output_data["table"]["rows"].append(
            {
                "heading": ag_sum.atom_group.id_str(),
                "message": row_message,
                "colour": row_colour,
                "columns": columns,
            }

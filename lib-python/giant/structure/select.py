...
from libtbx.utils import Failure, Sorry, null_out
...
def backbone(hierarchy, cache=None, copy=True, cbeta=False):
    ...
    if cbeta:
        sel = cache.selection(
            "pepnames and (name C or name CA or name N or name O or name CB)"
        )
    else:
        sel = cache.selection("pepnames and (name C or name CA or name N or name O)")
    return hierarchy.select(sel, copy_atoms=copy)
...
def sidechains(hierarchy, cache=None, copy=True, cbeta=True):
    ...
    if cbeta:
        sel = cache.selection(
            "(not element H) and pepnames and not (name C or name CA or name N or name O)"
        )
    else:
        sel = cache.selection(
            "(not element H) and pepnames and not (name C or name CA or name N or name O or name CB)"
        )
    return hierarchy.select(sel, copy_atoms=copy)
...
def default_secondary_structure_selections(hierarchy):
    ...
    result = (
        dssp.dssp(hierarchy, log=null_out(), out=null_out())
        .get_annotation()
        .as_atom_selections()
    )
    if len(result) == 0:
...
def default_secondary_structure_selections_filled(hierarchy):
    ...
    for c_id in chain_ids:
        ...
        for i, g in enumerate(c_sels):
            ...
            if sum(new_g_sel) == 0:
                logger.debug(
                    "No residues for this group that are not already in another group"
                )
                continue
        ...
        for i_this, v_this in enumerate(t_sel):
            ...
            if v_this != v_next:
                ...
                g_start = None
        logger.debug(
            "Filtered & Filled selections for chain {}:\n\t{}".format(
        ...
        for i, (start, end) in enumerate(f_sels):
            ...
            if g_size < 3:
                logger.debug(
                    "Group ({},{}) is less than three residues ({} residues)".format(
                        start, end, g_size
                    )
                )
                g_before = int(
                ...
                if g_before or g_after:
                    ...
                    if n_before:
                        logger.debug(
                            "Adding {} residues to previous group".format(n_before)
                        )
                    ...
                    logger.debug(
                        "Group of one residue with no neighbours -- removing group"
                    )
                    # Remove the group
                    f_sels[i] = None
                else:
                    logger.debug(
                        "No neighbouring groups -- leaving as group of two residues"
                    )

        # Remove the none groups
        o_sels = [(c_id, v[0], v[1]) for v in f_sels if v is not None]
        logger.debug(
            "Merged selections for chain {}:\n\t{}".format(
...
def get_select_function(selection):
    ...
    assert selection in functions, "Function not found. Choices: {}".format(
        ",".join(functions)
    )
    func = eval(selection)
    return func
...
def sel_altloc(hierarchy, altlocs=["", "A"], cache=None, copy=True):
    if not cache:
        cache = hierarchy.atom_selection_cache()
    sel = cache.selection(
        "(not element H) and ({})".format(
            " or ".join(['altid "{:1}"'.format(c) for c in altlocs])
        )
    )
    return hierarchy.select(sel, copy_atoms=copy)
...
def chain_conformer(chn, altlocs=["", "A"]):
    if isinstance(altlocs, str):
        altlocs = [altlocs]
    if not altlocs:
        altlocs = [""]
    assert (altlocs == [""]) or (
        (len(altlocs) - altlocs.count("")) == 1
    ), 'Supply up to one non-blank altloc (e.g. ["","A"], [""] or ["A"]): {}'.format(
        altlocs
    )
    return sel_altloc(
        hierarchy=iotbx.pdb.hierarchy.new_hierarchy_from_chain(chn), altlocs=altlocs
    ).only_chain()
...
def common_residues(chn_1, chn_2):
    ...
    alignment = align_sequences_default(
        seq_a=chn_1.as_sequence(), seq_b=chn_2.as_sequence()
    )
    # Flags for which residues to use
    m_seq_1, m_seq_2 = alignment.exact_match_selections()
    assert len(m_seq_1) == len(
        m_seq_2
    ), "Something has gone wrong: these should be the same length!"
    assert len(alignment.a) == len(
        alignment.b
    ), "Something has gone wrong: these should be the same length!"
    assert max(m_seq_1) < len(
        chn_1.as_sequence()
    ), "Something has gone wrong: selecting residue index greater than chain length"
    assert max(m_seq_2) < len(
        chn_2.as_sequence()
    ), "Something has gone wrong: selecting residue index greater than chain length"
...
def complete_backbone(chn, altlocs=["", "A"]):
    ...
    for i_rg, rg in enumerate(chn.residue_groups()):
        ...
        assert (
            len(confs) == 1
        ), "Something has gone wrong here. This should never be longer than 1."
...
def extract_backbone_atoms(residue, atoms=("N", "CA", "C")):
    ...
    residue_int = (
        residue.residue_name_plus_atom_names_interpreter().atom_name_interpretation.expected
    )

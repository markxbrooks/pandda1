...
class Alignment(object):
    ...
    def nat2ref(self, *args, **arg_dict):
        raise NotImplementedError(
            "Function not implemented for class of this type: {}".format(self.__class__)
        )
...
class LocalAlignment(CoordinateAlignment):
    ...
    def _validate_mappings(self, mappings):
        ...
        assert (
            not (mappings < 0).nonzero()[0].any()
        ), "Invalid alignment mapping: Cannot use mappings that are less than zero"
        assert (
            not (mappings > self.n_sites).nonzero()[0].any()
        ), "Invalid alignment mapping: Trying to use more mappings than there are alignments"
...
class MultipleLocalAlignment(LocalAlignment):
    ...
    def add(self, local_alignment, update=True):
        ...
        ), "Supplied object must be a LocalAlignment"

        assert (
            local_alignment not in self.local_alignments
        ), "Local alignment has already been added to object"
...
def align_structures_flexible(
    ...
    if require_hierarchies_identical:
        assert mov_hierarchy.is_similar_hierarchy(
            ref_hierarchy
        ), "Structures for alignment must have the same atoms (although atomic parameters can vary)"
...
def align_chains_flexible(
    ...
    assert (
        c_mov.atoms().extract_element() == c_ref.atoms().extract_element()
    ), "chn_mov and chn_ref must contain the same atoms"
    assert (
        c_mov.atoms().extract_name() == c_ref.atoms().extract_name()
    ), "chn_mov and chn_ref must contain the same atoms"
    ...
    for rg_mov in c_mov.residue_groups():
        ...
        rt_atm = superpose.least_squares_fit(
            reference_sites=xyz_ref_sel, other_sites=xyz_mov_sel
        ).rt()
...
def transform_coordinates_with_multiple_alignments(
    coordinates, alignments, mappings, inverse=False
):
    """
    ...
    out_coords = numpy.zeros(
        num_coords, dtype=[("x", float), ("y", float), ("z", float)]
    )

    # Iterate through the coords in groups and transform
    for key_or_idx, sel_idxs in itertools.groupby(
        list(range(num_coords)), key=lambda i: sorted_mappings[i]
    ):
...
def extract_sites_for_alignment(chain_obj):
    ...
    for resi in chain_obj.conformers()[0].residues():
        ...
        if (
            iotbx.pdb.common_residue_names_get_class(name=resi.resname)
            != "common_amino_acid"
        ):
            continue
...
def align_chains_rigid(mov_chain, ref_chain):
    ...
    lsq_rt = superpose.least_squares_fit(
        reference_sites=ref_sites_sel, other_sites=mov_sites_sel
    ).rt()

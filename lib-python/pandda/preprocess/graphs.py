    ...
    def plot(
        ...
        **kwargs
        ...
        axis.set_xticklabels(
            [round(t**-0.5, 1) if t > 0 else "" for t in axis.get_xticks()]
        )
...
class ResolutionDistributionPlotter(PanddaMultiDatasetPlotter):
    ...
    def plot(self, dataset_labels, dataset_list, dataset_dicts, *args, **kwargs):
...
class MakePanddaDatasetSummaryGraphs(object):
    ...
    def __init__(
        ...
        self.plotters = [
            WilsonPlotter(
                output_path_template=str(dataset_dir / "{label}-wilson.png"),
            ),
            ResolutionDistributionPlotter(
                output_path=str(output_dir / "dataset_resolutions.png"),
            ),
            RFactorDistributionPlotter(
                output_path=str(output_dir / "dataset_rfactors.png"),
            ),
            UnitCellDistributionPlotter(
                output_path=str(output_dir / "dataset_unitcells.png"),
            ),
        ]

...
def lcv_from_diagonals(uc1_diags, uc2_diags):
    chg_diags = fractional_change_for_diagonals(
        uc1_diags=uc1_diags, uc2_diags=uc2_diags
    )
...
def lcv_from_unit_cells(uc1, uc2):
    ...
    return lcv_from_unit_cell_parameters(
        uc1_params=uc1.parameters(), uc2_params=uc2.parameters()
    )
...
def pairwise_lcv(unit_cells=None, parameters=None):
    ...
    assert [unit_cells, parameters].count(
        None
    ) == 1, "Provide either unit_cells or parameters"
    if unit_cells:
        parameters = [uc.parameters() for uc in unit_cells]
    ...
    for i_uc1, i_uc2 in flex.nested_loop((num_objs, num_objs)):
        dist[i_uc1, i_uc2] = lcv_from_diagonals(
            uc1_diags=diags[i_uc1], uc2_diags=diags[i_uc2]
        )
    return dist
...
def pairwise_ecv(unit_cells=None, parameters=None):
    ...
    for i_uc1, uc1 in enumerate(parameters):
        diags[i_uc1] = unit_cell_diagonals(*uc1)
    for i_uc1, i_uc2 in flex.nested_loop((num_objs, num_objs)):
        dist[i_uc1, i_uc2] = flex.double(
            fractional_change_for_diagonals(diags[i_uc1], diags[i_uc2])
        ).norm()
    return dist

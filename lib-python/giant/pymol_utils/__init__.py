...
class _PymolScript(object):
        ...
        for prefix in ["_cmd_"]:
            for attribute_name in dir(cls):
                ...
                if hasattr(cls, func_name):
                    continue
                func = cls._make_method(
                    func_name=func_name, template_name=attribute_name
                )
    ...
    def _append(self, thing):
        if isinstance(thing, list):
            [self._append(t) for t in thing]
        elif isinstance(thing, str):
            self.lines.append(thing)
        else:
            raise Exception("Invalid type: {}".format(type(thing)))
    ...
    def set(self, setting, *args, **kwargs):
        # TODO: Implement PyMOL set command construction
    ...
    def custom(self, function, *args, **kwargs):
        # TODO: Implement PyMOL custom command construction
        # This method appears to be incomplete
        pass
    ...
    def load_pdb(self, f_name, obj=None, state=0):
        ...
        if obj is None:
            obj = os.path.basename(os.path.splitext(f_name)[0])
    ...
    def make_mesh(self, obj, mesh_suffix=".mesh", contour_level=1, colour=None):
        ...
        self._append(
            self._isomesh.format(obj=mesh_name, map=obj, contour_level=contour_level)
        )
        if colour is not None:
            self.colour(obj=mesh_name, colour=colour)
        return mesh_name
    ...
    def colour(self, obj, colour=None):
        ...
        cmd = self._format(
            template=self._man_colour, colour=repr(colour), obj=repr(obj)
        )
        self._append(cmd)
        return colour
...
class PythonScript(_PymolScript):
    ...
    _cmd_rebuild = (
        'cmd.rebuild(selection="{selection}", representation="{representation}")'
    )

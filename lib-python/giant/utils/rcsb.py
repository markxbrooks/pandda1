...
def download_structure_factors(pdb_id, filename, zipped=True):
    ...
    try:
        if zipped:
            tempfile = wget.download(
                "https://files.rcsb.org/download/{}-sf.cif.gz".format(pdb_id)
            )
        else:
            tempfile = wget.download(
                "https://files.rcsb.org/download/{}-sf.cif".format(pdb_id)
            )
    except:
        logger("Failed to download:{}".format(pdb_id))
        raise
...
def download_and_organise_pdb_as_necessary(pdb_id, out_dir="pdb-download"):
    ...
    if not os.path.exists(pdb_file):
        download_pdb(pdb_id=pdb_id, filename=pdb_file)
    return pdb_file

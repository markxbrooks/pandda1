...
def resolve_glob(path, n=None):
    ...
    if (n is not None) and (len(glob_list) != n):
        raise ValueError(
            "glob did not return expected number of results ({}): {}".format(
                n, glob_list
            )
        )
    if n == 1:
...
def splice_ext(path, new, position=-1):
    ...
    assert abs(position) <= len(
        split
    ), 'invalid position selected ({}) to insert "{}" into {}'.format(
        position, new, split
    )
...
def decompress_all_files_in_directory(
    path, recursive=True, extension=".gz", delete_original=True
):
    """Decompress in-place all wrappers in a directory with a particular extension"""
    ...
    for cur_dir, _, cur_files in os.walk(path):
        for item in cur_files:
            ...
            assert not os.path.exists(f_out), "output file already exists: {}".format(
                f_out
            )
            if extension == ".gz":
                import gzip

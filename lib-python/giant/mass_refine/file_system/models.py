...
class MakeModelFolder(object):
    ...
    def __call__(
        ...
        assert not self.f.root_dir.exists(), "model_folder {} already exists".format(
            str(self.f.root_dir),
        )
    ...
    def copy_cif_restraints(self, paths):
        ...
        for i, p in enumerate(paths):
            ...
            opath = self.f.cif_dir / ("{i:02d}-{p}".format(i=i + 1, p=ipath.name))
...
class ModelFolder(FolderWithMeta):
    ...
    def __init__(self, root_dir):
        ...
        self.model_path = self.root_dir / "model.pdb"

        self.cif_dir = self.root_dir / "cif_files"
    ...
    def model(self):
        ...
        if self.model_path.exists():
            ...
            raise IOError("model does not exist: {}".format(self.model_path))

    def refinement_parameters(self):

        ref_params = sorted(self.root_dir.glob("*.params"))
...
class ModelsFileSystem(object):
    ...
    def __str__(self):
        ...
        s_ = (
            ...
            name=self.name,
            path=self.root_dir,
            models_string="\n".join([str(r) for r in self.get_all()]).replace(
                "\n", "\n| \t"
            ),
        )

...
class Arrow(object):
    ...
    def __init__(
        ...
        colors=None,
        radius=None,
        head_length_fraction=0.4,
        head_radius_fraction=1.5,
    ):
        if colors is None:
            colors = [[1, 0, 0], [1, 0, 0]]
        if radius is None:
        ...
        assert self.head_length_fraction >= 0.0
        assert self.head_length_fraction <= 1.0
        assert self.head_radius_fraction > 0.0
    ...
    def make_components(self):
        ...
        mid = tuple(
            start[i] + body_length_fraction * (end[i] - start[i]) for i in range(3)
        )

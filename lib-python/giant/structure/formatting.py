...
class _Selection(object):
    ...
    def format(cls, obj):
        return cls.join_and(
            [s for s in cls._format_any_to_list(obj) if s not in cls.remove]
        )

    @classmethod
    def join_or(cls, strings, extra_join=""):
        return (cls._join_or + extra_join).join(
            [cls._sep_or.format(s) for s in strings]
        )

    @classmethod
    def join_and(cls, strings, extra_join=""):
        return (cls._join_and + extra_join).join(
            [cls._sep_and.format(s) for s in strings]
        )
    ...
    def _format_dict(cls, obj_dict):
        return [
            cls.__dict__.get(l).format(obj_dict.get(l))
            for l in cls._labels
            if (l in obj_dict)
        ]

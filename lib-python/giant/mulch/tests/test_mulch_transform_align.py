...
def test_AlignDatasets(five_baz2b_test_datasets_mcd_labelled):
    ...
    five_baz2b_test_datasets_mcd_labelled = copy.deepcopy(
        five_baz2b_test_datasets_mcd_labelled
    )
    ...
    if True:
        ...
        assert (
            d_alignment.local_alignments[0].alignment_mxs[14]
            is d_alignment.alignment_mxs[14]
        )
        ...
        assert list(d_alignment.alignment_mxs[14].t) == pytest.approx(
            [-0.579589, -0.544498, 1.165509],
            abs=1e-6,
        )

        test_site = (24.132558207982353, 18.676095002909786, 24.872317348333294)

        assert test_site == pytest.approx(tuple(d_alignment.alignment_sites_ref[17]))
        ...
        assert test_site == pytest.approx(
            tuple(
                (d_alignment.alignment_mxs[17].r * d_alignment.alignment_sites_nat[17])
                + d_alignment.alignment_mxs[17].t
            )
        )
    ...
    if True:
        ...
        assert test_site == pytest.approx(
            tuple(
                (d_alignment.alignment_mx.r * d_alignment.alignment_sites_nat[17])
                + d_alignment.alignment_mx.t
            )
        )

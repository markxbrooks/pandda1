...
from giant.html import divs, objects
...
class MakePanddaDatasetSummaryHtml(object):
    ...
    def get_input_summary(
        ...
        empty_directories = sorted(info_dict["empty_directories"])

        filtered_files = sorted(info_dict["filtered_files"].items())

        rejected_datasets = sorted(info_dict["rejected_datasets"].items())

        loaded_datasets = sorted(info_dict["loaded_datasets"])
            ...
            for reason, keys in sorted(filtered_files_counts.items()):
                block.append(
                    divs.Block(
                        ...
                        text=" ".join([d_label.format(k=k) for k in keys]),
                    )
                )
            ...
            panel.append(
                divs.Panel(
                    ...
                    contents=[
                        divs.Block(
                            text=" ".join(
                                [d_label.format(k=k) for k in loaded_datasets]
                            ),
                        ),
                    ],
                )
        ...
        main_block = divs.Panel(
            ...
            contents=[
                self.make_standard_block(
                    title=("Distribution of dataset resolutions."),
                    width=width,
                    image=output_graphs.get("resolution_distribution"),
                ),
                self.make_standard_block(
                    title=("Distributions of dataset R-factors."),
                    width=width,
                    image=output_graphs.get("rfactor_distribution"),
                ),
                self.make_standard_block(
                    title=("Distributions of dataset unit cells."),
                    width=width,

...
class FolderWithMeta(object):
    ...
    def tmp_dir(self):
        ...
        tmp_dir = self.root_dir / "tmp_files"
...
class DirectoryPathGenerator(object):
    ...
    def get_next(self, suffix=None):
        ...
        new_path = self.root_dir / (
            str(self.prefix)
            + "_"
            + str(self.int_format).format(next_int)
            + "_"
            + str(suffix)
        )
    ...
    def get_all_as_dict_by_int(self):
        ...
        return {self.get_int_from_name(p.name): p for p in self.get_all()}

    def get_all_as_dict_by_name(self):

        return {self.get_label_from_name(p.name): p for p in self.get_all()}
    ...
    def get_int_from_name(self, name):
        ...
        return int(name.split("_")[1])  # 0prefix_1int_2suffix

    def get_label_from_name(self, name):

        return "_".join(name.split("_")[2:])  # 0prefix_1int_2suffix
